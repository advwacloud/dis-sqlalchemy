...
pytest, dis_sqlalchemy itself is adding a large series of option and
customizations to the pytest runner using plugin points, to allow for
dis_sqlalchemy's multiple database support, database setup/teardown and
...
of what dis_sqlalchemy tests is creating and dropping lots of tables
...
When multiprocessing is used, the dis_sqlalchemy testing framework will create a
...
The dis_sqlalchemy test can run against database running in Docker containers.
...
quickly set up a local environment for dis_sqlalchemy development. They are **NOT**
intended for production use!

**PostgreSQL configuration**::

    # create the container with the proper configuration for dis_sqlalchemy
...
CONFIGURING LOGGING
-------------------
dis_sqlalchemy logs its activity and debugging through Python's logging package.
Any log target can be directed to the console with command line options, such
as::

    $ ./pytest test/orm/test_unitofwork.py -s \
      --log-debug=dis_sqlalchemy.pool --log-info=dis_sqlalchemy.engine
