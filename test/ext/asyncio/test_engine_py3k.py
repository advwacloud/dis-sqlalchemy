...
from dis_sqlalchemy.ext.asyncio import async_engine_from_config
from dis_sqlalchemy.ext.asyncio import create_async_engine
from dis_sqlalchemy.ext.asyncio import create_async_pool_from_url
from dis_sqlalchemy.ext.asyncio import engine as _async_engine
from dis_sqlalchemy.ext.asyncio import exc as async_exc
from dis_sqlalchemy.ext.asyncio import exc as asyncio_exc
...
from dis_sqlalchemy.pool import AsyncAdaptedQueuePool
...
from dis_sqlalchemy.util import greenlet_spawn
...
class AsyncEngineTest(EngineFixture):
    ...
    def test_appropriate_warning_for_gced_connection(
        ...
        from dis_sqlalchemy.util.concurrency import await_only
        ...
        if simulate_gc:
            ...
            from dis_sqlalchemy.pool.base import _finalize_fairy

            with mock.patch.object(
                pool._dialect,
                "do_rollback",
                mock.Mock(side_effect=Exception("can't run rollback")),
            ), mock.patch("dis_sqlalchemy.util.warn") as m:
                _finalize_fairy(
            ...
            with mock.patch("dis_sqlalchemy.util.warn") as m:
    ...
    def test_async_engine_from_config(self):
        config = {
            "dis_sqlalchemy.url": testing.db.url.render_as_string(
                hide_password=False
            ),
            "dis_sqlalchemy.echo": "true",
        }
    ...
    async def test_async_creator_accepts_args_if_called_directly(
        ...
        https://github.com/dis_sqlalchemy/dis_sqlalchemy/issues/8215#issuecomment-1181791539.
...
class AsyncCreatePoolTest(fixtures.TestBase):
    ...
    def mock_create(self):
        with patch(
            "dis_sqlalchemy.ext.asyncio.engine._create_pool_from_url",
        ) as p:
            yield p
