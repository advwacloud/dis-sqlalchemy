...
from dis_sqlalchemy.sql.expression import ColumnElement
...
class Interval(Base):
    ...
    def old_radius(cls) -> ColumnElement[float]:
        ...
        if typing.TYPE_CHECKING:
            # EXPECTED_RE_TYPE: dis_sqlalchemy.*Function\[builtins.float\*?\]
            reveal_type(f1)

            # EXPECTED_RE_TYPE: dis_sqlalchemy.*ColumnElement\[builtins.float\*?\]
            reveal_type(expr)
        return expr
...
if typing.TYPE_CHECKING:
    ...
    # EXPECTED_RE_TYPE: dis_sqlalchemy.*._HybridClassLevelAccessor\[builtins.int\*?\]
    reveal_type(Interval.length)

    # EXPECTED_RE_TYPE: dis_sqlalchemy.*._HybridClassLevelAccessor\[builtins.float\*?\]
