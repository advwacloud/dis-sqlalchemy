...
dis_sqlalchemy supports MySQL starting with version 5.0.2 through modern releases,
...
Within the realm of dis_sqlalchemy, the two databases have a small number of
syntactical and behavioral differences that dis_sqlalchemy accommodates automatically.
To connect to a MariaDB database, no changes to the database URL are required::


    engine = create_engine("mysql+pymysql://user:pass@some_mariadb/dbname?charset=utf8mb4")

Upon first connect, the dis_sqlalchemy dialect employs a
...
all lower case both within dis_sqlalchemy as well as on the MySQL / MariaDB
...
When creating tables, dis_sqlalchemy will automatically set ``AUTO_INCREMENT`` on
...
dis_sqlalchemy itself cannot render this ``_binary`` prefix reliably, as it does
...
Many of the MySQL / MariaDB SQL extensions are handled through dis_sqlalchemy's generic
...
dis_sqlalchemy provides ``ON DUPLICATE KEY UPDATE`` support via the MySQL-specific
...
dis_sqlalchemy standardizes the DBAPI ``cursor.rowcount`` attribute to be the
usual definition of "number of rows matched by an UPDATE or DELETE" statement.
This is in contradiction to the default setting on most MySQL DBAPI drivers,
which is "number of rows actually modified/deleted".  For this reason, the
dis_sqlalchemy MySQL dialects always add the ``constants.CLIENT.FOUND_ROWS``
...
become part of the index. dis_sqlalchemy provides this feature via the
...
an index. dis_sqlalchemy provides this feature via the
...
an index or primary key constraint. dis_sqlalchemy provides this feature via the
...
by dis_sqlalchemy in conjunction with the MySQL or MariaDB backends.  This argument is
...
:class:`~dis_sqlalchemy.ForeignKeyConstraint` at reflection time::
...
dis_sqlalchemy supports both the :class:`.Index` construct with the
...
dis_sqlalchemy's MySQL dialect does not yet have an option to generate
MySQL's "ON UPDATE CURRENT_TIMESTAMP" clause, noting that this is not a general
purpose "ON UPDATE" as there is no such syntax in standard SQL.  dis_sqlalchemy's
...
parameter and pass a textual clause that also includes the ON UPDATE clause::
    ...
    from dis_sqlalchemy import Table, MetaData, Column, Integer, String, TIMESTAMP
...
specify this new flag, dis_sqlalchemy emits the "NULL" specifier explicitly with
...
dis_sqlalchemy also emits NOT NULL for TIMESTAMP columns that do specify
``nullable=False``.   The following example illustrates::

    from dis_sqlalchemy import MetaData, Integer, Table, Column, text
    from dis_sqlalchemy.dialects.mysql import TIMESTAMP
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    def get_column_specification(self, column, **kw):
        ...
        # see: https://docs.dis_sqlalchemy.org/en/latest/dialects/mysql.html#mysql_timestamp_null  # noqa
        ...
        if self.is_mariadb:
            ...
            if mdb_version > (10, 2) and mdb_version < (10, 2, 9):
                util.warn(
                    ...
                    "with dis_sqlalchemy's boolean datatype (MDEV-13596). An "
