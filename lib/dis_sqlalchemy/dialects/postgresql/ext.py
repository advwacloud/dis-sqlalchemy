...
class to_tsvector(_regconfig_fn):
    ...
    ``from dis_sqlalchemy.dialects import postgresql``, or by creating a PostgreSQL
    engine using ``create_engine("postgresql...")``,
    :class:`_postgresql.to_tsvector` will be used automatically when invoking
    ``dis_sqlalchemy.func.to_tsvector()``, ensuring the correct argument and return
...
class to_tsquery(_regconfig_fn):
    ...
    ``dis_sqlalchemy.func.to_tsquery()``, ensuring the correct argument and return
    type handlers are used at compile and execution time.
...
class plainto_tsquery(_regconfig_fn):
    ...
    invoking ``dis_sqlalchemy.func.plainto_tsquery()``, ensuring the correct
...
class phraseto_tsquery(_regconfig_fn):
    ...
    invoking ``dis_sqlalchemy.func.phraseto_tsquery()``, ensuring the correct
    argument and return type handlers are used at compile and execution time.
...
class websearch_to_tsquery(_regconfig_fn):
    ...
    invoking ``dis_sqlalchemy.func.websearch_to_tsquery()``, ensuring the correct
    argument and return type handlers are used at compile and execution time.
...
class ts_headline(_regconfig_fn):
    ...
    ``dis_sqlalchemy.func.ts_headline()``, ensuring the correct argument and return
    type handlers are used at compile and execution time.
