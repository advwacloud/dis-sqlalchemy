...
PostgreSQL supports sequences, and dis_sqlalchemy uses these as the default means
of creating new primary key values for integer-based primary key columns. When
creating tables, dis_sqlalchemy will issue the ``SERIAL`` datatype for
integer-based primary key columns, which generates a sequence and server side
default corresponding to the column.

To specify a specific named sequence to be used for primary key generation,
use the :func:`~dis_sqlalchemy.schema.Sequence` construct::
...
When dis_sqlalchemy issues a single INSERT statement, to fulfill the contract of
...
:func:`~dis_sqlalchemy.sql.expression.insert()` construct is executed using
...
.. note::
   ...
   Previous versions of dis_sqlalchemy did not have built-in support for rendering
...
Most dis_sqlalchemy dialects support setting of transaction isolation level
...
:ref:`postgresql_schema_reflection`, dis_sqlalchemy is generally organized around
...
dis_sqlalchemy will by default use the return value of ``pg_get_constraintdef()``
in order to determine the remote schema name.  That is, if our ``search_path``
were set to include ``test_schema``, and we invoked a table
reflection process as follows::

    >>> from dis_sqlalchemy import Table, MetaData, create_engine, text
    ...
    ...                       autoload_with=conn)
    ...
    <dis_sqlalchemy.engine.result.CursorResult object at 0x101612ed0>
...
:meth:`_schema.MetaData.reflect`::
    ...
    ...
    <dis_sqlalchemy.engine.result.CursorResult object at 0x1016126d0>
...
dis_sqlalchemy provides ``ON CONFLICT`` support via the PostgreSQL-specific
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument may
  also refer to a dis_sqlalchemy construct representing a constraint,
...
The :class:`_postgresql.TSVECTOR` type can provide for explicit CAST::
    ...
    from dis_sqlalchemy.dialects.postgresql import TSVECTOR
    from dis_sqlalchemy import select, cast
...
assuming the ``dis_sqlalchemy.dialects.postgresql`` package has been imported,
...
PostgreSQL to ensure that you are generating queries with dis_sqlalchemy that
...
syntaxes. It uses dis_sqlalchemy's hints mechanism::
...
* ``NOT VALID``:  This option applies towards CHECK and FOREIGN KEY constraints
  ...
  When using a SQL migration tool such as `Alembic <https://alembic.dis_sqlalchemy.org>`_
...
datatypes.  dis_sqlalchemy's SQL expression language has native support for
...
forms. These classes of SQL function calling forms in dis_sqlalchemy are available
...
* ``json_populate_record()``:
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import select, func, literal_column
    >>> stmt = select(
...
* ``json_to_record()`` - this form uses a PostgreSQL specific form of derived
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import select, func, column, Integer, Text
    >>> stmt = select(
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import table, column, ARRAY, Integer
    >>> from dis_sqlalchemy import select, func
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import table, column, func, tuple_
...
refers towards as a "record" type. dis_sqlalchemy :class:`_sql.FromClause` objects
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import table, column, func, select
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            ...
            raise exc.CompileError(
                ...
                f'"dis_sqlalchemy.func.{element.name}()" to ensure registration '
