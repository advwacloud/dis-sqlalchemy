...
# Copyright (C) 2005-2023 the dis_sqlalchemy authors and contributors <see AUTHORS
# file>
#
...
The asyncpg dialect is dis_sqlalchemy's first Python asyncio dialect.

Using a special asyncio mediation layer, the asyncpg dialect is usable
as the backend for the :ref:`dis_sqlalchemy asyncio <asyncio_toplevel>`
extension package.
...
adding the dis_sqlalchemy-specific flag ``async_fallback`` to the URL
...
.. note::
    ...
    returns them as strings. dis_sqlalchemy sets default type decoder for ``json``
...
The asyncpg dis_sqlalchemy dialect makes use of ``asyncpg.connection.prepare()``
...
is implemented by dis_sqlalchemy, it is part of the DBAPI emulation portion of the
...
.. warning::  The ``asyncpg`` database driver necessarily uses caches for
   ...
   prepared statements themselves which are optionally cached by dis_sqlalchemy's
   ...
   The dis_sqlalchemy asyncpg dialect will invalidate these caches within its local
   ...
   operated upon the previous structures. The dis_sqlalchemy asyncpg dialect will
...
.. seealso::
   ...
   https://github.com/dis_sqlalchemy/dis_sqlalchemy/issues/6467
...
class AsyncAdapt_asyncpg_dbapi:
    ...
    class InvalidCachedStatementError(NotSupportedError):
        def __init__(self, message):
            super().__init__(
                message + " (dis_sqlalchemy asyncpg dialect will now invalidate "
                "all prepared caches in response to this exception)",
            )

    # pep-249 datatype placeholders.  As of dis_sqlalchemy 2.0 these aren't
