...
enough from its ``psycopg2`` predecessor that dis_sqlalchemy supports it
...
The dis_sqlalchemy ``psycopg`` dialect provides both a sync and an async
...
.. seealso::
    ...
    :ref:`postgresql_psycopg2` - The dis_sqlalchemy ``psycopg``
...
class AsyncAdapt_psycopg_cursor:
    ...
    def execute(self, query, params=None, **kw):
        ...
        # dis_sqlalchemy result is not async, so need to pull all rows here
