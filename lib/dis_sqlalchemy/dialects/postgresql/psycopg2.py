...
Keyword arguments that are specific to the dis_sqlalchemy psycopg2 dialect
...
portion of the URL is omitted, dis_sqlalchemy passes ``None`` to psycopg2,
...
the URL, dis_sqlalchemy will create a single string of the host and port
...
dis_sqlalchemy implements a native form of the "insert many values"
...
.. versionadded:: 2.0 Replaced psycopg2's ``execute_values()`` fast execution
   helper with a native dis_sqlalchemy mechanism referred towards as
...
* ``values_only`` - this is the default value.  dis_sqlalchemy's native
...
* ``'values_plus_batch'``- dis_sqlalchemy's native
  :ref:`insertmanyvalues <engine_insertmanyvalues>` handler is used for qualifying
  INSERT statements, assuming
...
* For all PostgreSQL versions, psycopg2 supports a client-side encoding
  ...
  established.  The dis_sqlalchemy psycopg2 dialect supports this using the
...
via the ``dis_sqlalchemy.dialects.postgresql`` logger.  When this logger
is set to the ``logging.INFO`` level, notice messages will be logged::

    import logging

    logging.getLogger('dis_sqlalchemy.dialects.postgresql').setLevel(logging.INFO)
...
the HSTORE type.   The dis_sqlalchemy psycopg2 dialect will enable this extension
...
place within dis_sqlalchemy's own marshalling logic, and not that of ``psycopg2``
...
logger = logging.getLogger("dis_sqlalchemy.dialects.postgresql")
...
class PGExecutionContext_psycopg2(_PGExecutionContext_common_psycopg):
    ...
    def _log_notices(self, cursor):
        ...
        # around test suites such as dis_sqlalchemy's using a Mock object for
        # cursor
