...
`datetime` objects and a SQLite-supported format. dis_sqlalchemy's own
:class:`~dis_sqlalchemy.types.DateTime` and related types provide date formatting
...
* SQLite also has an explicit "AUTOINCREMENT" keyword, that is **not**
  ...
  recommended for general use.  dis_sqlalchemy does not render this keyword
...
Allowing autoincrement behavior dis_sqlalchemy types other than Integer/INTEGER
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. seealso::
    ...
    :ref:`dis_sqlalchemy.ext.compiler_toplevel`
...
dis_sqlalchemy ORM.  dis_sqlalchemy's :class:`.Session` object by default runs
...
dis_sqlalchemy ties into this PRAGMA statement using the
...
begun.   dis_sqlalchemy's SAVEPOINT support is available using the
...
* The ``PRAGMA foreign_keys = ON`` statement must be emitted on all
  ...
  :meth:`dis_sqlalchemy.schema.MetaData.create_all`.

dis_sqlalchemy allows for the ``PRAGMA`` statement to be emitted automatically for
...
.. seealso::
    ...
    :ref:`event_toplevel` - dis_sqlalchemy event API.

    :ref:`use_alter` - more information on dis_sqlalchemy's facilities for handling
     mutually-dependent foreign key constraints.
...
dis_sqlalchemy renders the PRIMARY KEY constraint separately, so the conflict
...
dis_sqlalchemy provides ``ON CONFLICT`` support via the SQLite-specific
...
dis_sqlalchemy's reflection process, when inspecting types, uses a simple
lookup table to link the keywords returned to provided dis_sqlalchemy types.
...
requires that dis_sqlalchemy filter out these dots in result sets.

The bug, entirely outside of dis_sqlalchemy, can be illustrated thusly::
...
dis_sqlalchemy relies upon column names being predictable in how they match
to the original statement, so the dis_sqlalchemy dialect has no choice but
to filter these out::
...
Note that above, even though dis_sqlalchemy filters out the dots, *both
...
Therefore, the workaround applied by dis_sqlalchemy only impacts
...
.. versionadded:: 2.0  Added the ``sqlite_include_internal=True`` parameter.
   Previously, these tables were not ignored by dis_sqlalchemy reflection
   methods.
...
class DATETIME(_DateTimeMixin, sqltypes.DateTime):
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        import re
        from dis_sqlalchemy.dialects.sqlite import DATETIME
...
class TIME(_DateTimeMixin, sqltypes.Time):
    ...
    ``storage_format`` and ``regexp`` parameters, such as::
        ...
        import re
        from dis_sqlalchemy.dialects.sqlite import TIME
...
class SQLiteDialect(default.DefaultDialect):
        ...
        if self.dbapi is not None:
            if self.dbapi.sqlite_version_info < (3, 7, 16):
                util.warn(
                    ...
                    "more complex ORM scenarios.  dis_sqlalchemy 1.4 and above "
            ...
            # see https://www.dis_sqlalchemy.org/trac/ticket/2568
    ...
    def _resolve_type_affinity(self, type_):
        ...
        This method allows dis_sqlalchemy to support that algorithm, while still
