...
In addition to the above DBAPI layers with native dis_sqlalchemy support, there
...
table. dis_sqlalchemy considers ``IDENTITY`` within its default "autoincrement"
...
:class:`.Sequence` - and will generate DDL as such::
    ...
    from dis_sqlalchemy import Table, MetaData, Column, Integer
...
.. note::
    ...
    ``autoincrement=True`` to enable the IDENTITY keyword, dis_sqlalchemy does not
...
.. note::
    ...
    enabled.   The dis_sqlalchemy SQL Server dialect will perform this operation
...
parameters passed to the :class:`_schema.Identity` object::
    ...
    from dis_sqlalchemy import Table, Integer, Column, Identity
...
.. versionchanged:: 1.3.19  The :class:`_schema.Identity` object is
   ...
   functional in the normal way starting from dis_sqlalchemy version 1.4.
...
implement this pattern smoothly in dis_sqlalchemy, the primary datatype of the
...
for a given ``IDENTITY`` column, a process which dis_sqlalchemy performs
...
* In the vast majority of cases, RETURNING is used in conjunction with INSERT
  ...
  As of dis_sqlalchemy 2.0, the :ref:`engine_insertmanyvalues` feature is also
  ...
  .. versionchanged:: 2.0.10 The :ref:`engine_insertmanyvalues` feature for
     SQL Server was temporarily disabled for dis_sqlalchemy version 2.0.9 due to
...
that refers to the identity column explicitly.  The dis_sqlalchemy dialect will
...
how dis_sqlalchemy handles this:
...
"OFFSET n ROWS" and "FETCH NEXT n ROWS" clauses.  dis_sqlalchemy supports these
...
by the dis_sqlalchemy :class:`.Engine` object includes
...
Server in a future release.   dis_sqlalchemy normally relates these types to the
...
* Complete control over whether the "old" or "new" types are rendered is
  available in all dis_sqlalchemy versions by using the UPPERCASE type objects
...
a backwards compatibility mode dis_sqlalchemy may attempt to use T-SQL
...
dis_sqlalchemy by default uses OUTPUT INSERTED to get at newly
...
OUTPUT INSERTED is used.    Previous versions of dis_sqlalchemy therefore had
...
dis_sqlalchemy 2.0 now retrieves the "rowcount" manually for these particular use
...
impacted by it. As of dis_sqlalchemy 2.0.5, ORM versioning has been fully
...
class MSTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_JSON(self, type_, **kw):
        # this is a bit of a break with dis_sqlalchemy's convention of
