...
database using Azure credentials::
    ...
    def provide_token(dialect, conn_rec, cargs, cparams):
        # remove the "Trusted_Connection" parameter that dis_sqlalchemy adds
...
.. tip::
    ...
    The ``Trusted_Connection`` token is currently added by the dis_sqlalchemy
...
and that can cause problems with dis_sqlalchemy's "autobegin" (and implicit
...
dis_sqlalchemy itself.  As dis_sqlalchemy has its own pooling behavior, it is often
...
dis_sqlalchemy engine itself fully discards a connection or if the engine is
disposed.
...
Previous limitations with the dis_sqlalchemy ORM's "versioned rows" feature with
Pyodbc have been resolved as of dis_sqlalchemy 2.0.5. See the notes at
...
cursor when an executemany call is to be used.   The dis_sqlalchemy PyODBC SQL
...
.. versionchanged:: 2.0.9 - the ``fast_executemany`` parameter now has its
   ...
   include RETURNING.  Previously, dis_sqlalchemy 2.0's :term:`insertmanyvalues`
