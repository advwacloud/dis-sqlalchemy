...
then connect in dis_sqlalchemy using the ``service_name`` query string parameter::
...
Options consumed by the dis_sqlalchemy cx_Oracle dialect outside of the driver
--------------------------------------------------------------------------

There are also options that are consumed by the dis_sqlalchemy cx_oracle dialect
...
be used in place of dis_sqlalchemy's pooling functionality.  This can be achieved
...
dis_sqlalchemy dialect to use NCHAR/NCLOB for the :class:`.Unicode` /
...
``Cursor.var()``, as well as dis_sqlalchemy's own decoding function, as the
...
possible for dis_sqlalchemy to know exactly how data should be bound, as some
...
On the dis_sqlalchemy side, the :meth:`.DialectEvents.do_setinputsizes` event can
...
dis_sqlalchemy perspective before they are converted to the raw ``setinputsizes()``
...
objects which have a ``.key`` and a ``.type`` attribute::
    ...
    def _log_setinputsizes(inputsizes, cursor, statement, parameters, context):
        for bindparam, dbapitype in inputsizes.items():
                log.info(
                    "Bound parameter name: %s  dis_sqlalchemy type: %r  "
                    "DBAPI object: %s",
...
however is set by default for the ``Text`` type within the dis_sqlalchemy 1.2
...
datatypes to be delivered as a single buffer. As such, dis_sqlalchemy makes use of
...
to support this system is not implemented in dis_sqlalchemy.
...
dis_sqlalchemy's numeric types can handle receiving and returning values as Python
...
use, as well as if no dis_sqlalchemy typing objects are present.   There are
...
dis_sqlalchemy type (or a subclass of such).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dbapi_module is not None:
            # these constants will first be seen in dis_sqlalchemy datatypes
