...
dis_sqlalchemy Table objects which include integer primary keys are usually
...
feature, dis_sqlalchemy relies upon sequences to produce these values.   With the
...
specify sequences, use the dis_sqlalchemy.schema.Sequence object which is passed
...
names using UPPERCASE text.   dis_sqlalchemy on the other hand considers an
...
an UPPERCASE name on the dis_sqlalchemy side indicates a case sensitive
identifier, and dis_sqlalchemy will quote the name - this will cause mismatches
...
lowercase names should be used on the dis_sqlalchemy side.
...
greater it is now 128.   This change impacts dis_sqlalchemy in the area of
...
name = 'compatible';``.   The dis_sqlalchemy Oracle dialect, when tasked with
...
As of dis_sqlalchemy 1.4, the default max identifier length for the Oracle dialect
...
dis_sqlalchemy to change this default conservatively.   For example, the following
...
Applications which have run versions of dis_sqlalchemy prior to 1.4 on an  Oracle
...
* dis_sqlalchemy currently makes use of ROWNUM to achieve
...
(that is, a ``cursor.execute()`` style statement; dis_sqlalchemy does not generally
...
When using the dis_sqlalchemy ORM, the ORM has limited ability to manually issue
...
.. warning:: The status of Oracle 8 compatibility is not known for dis_sqlalchemy
   2.0.
...
* the NVARCHAR2 and NCLOB datatypes are no longer generated as DDL when
  the :class:`~dis_sqlalchemy.types.Unicode` is used - VARCHAR2 and CLOB are issued
  instead. This because these types don't seem to work correctly on Oracle 8
  even though they are available. The :class:`~dis_sqlalchemy.types.NVARCHAR` and
  :class:`~dis_sqlalchemy.dialects.oracle.NCLOB` types will always generate
  NVARCHAR2 and NCLOB.
...
Bitmap indexes cannot be unique and cannot be compressed. dis_sqlalchemy will not
