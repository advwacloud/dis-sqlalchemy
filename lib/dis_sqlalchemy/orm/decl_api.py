        ...
        if reg is None:
            ...
            if not isinstance(reg, registry):
                raise exc.InvalidRequestError(
                    ...
                    "or registry is not a dis_sqlalchemy.orm.registry() object"
                )
            else:
...
def synonym_for(
    ...
    The :ref:`hybrid properties <mapper_hybrids>` feature of dis_sqlalchemy
...
def declarative_mixin(cls: Type[_T]) -> Type[_T]:
    ...
    dis_sqlalchemy declarative mixin classes when no other context is present.
    ...
    if reg is not None:
        if not isinstance(reg, registry):
            raise exc.InvalidRequestError(
                ...
                "not an instance of dis_sqlalchemy.orm.registry()"
            )
...
class MappedAsDataclass(metaclass=DCTransformDeclarative):
    ...
    .. seealso::
        ...
        on dis_sqlalchemy native dataclass mapping
...
def declarative_base(
    *,
    ...
    appropriate :class:`~dis_sqlalchemy.schema.Table` objects and makes
    ...
    .. versionchanged:: 1.4  The :func:`_orm.declarative_base`
       ...
       ``dis_sqlalchemy.orm`` package from the ``declarative.ext`` package.


    :param metadata:
      An optional :class:`~dis_sqlalchemy.schema.MetaData` instance.  All
      :class:`~dis_sqlalchemy.schema.Table` objects implicitly declared by
      subclasses of the base will share this MetaData.  A MetaData instance
      will be created if none is provided.  The
      :class:`~dis_sqlalchemy.schema.MetaData` instance will be available via the
    ...
    :param type_annotation_map: optional dictionary of Python types to
        dis_sqlalchemy :class:`_types.TypeEngine` classes or instances.  This
...
class registry:
    ...
    * :meth:`_orm.registry.map_imperatively` will produce a
      ...
      provided by the ``dis_sqlalchemy.orm.mapper()`` classical mapping function,
      which is removed as of dis_sqlalchemy 2.0.
    ...
    def __init__(
        ...
        *,
        ...
        :param type_annotation_map: optional dictionary of Python types to
          dis_sqlalchemy :class:`_types.TypeEngine` classes or instances.
    ...
    def map_imperatively(
        ...
        **kw: Any,
        ...
        dis_sqlalchemy ``mapper()`` function, except that it's in terms of
