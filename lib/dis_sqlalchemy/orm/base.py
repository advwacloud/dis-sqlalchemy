...
class NotExtension(InspectionAttrExtensionType):
    ...
    not part of dis_sqlalchemy.ext.
...
if TYPE_CHECKING:
    ...
    # these can be replaced by dis_sqlalchemy.ext.instrumentation
...
def object_mapper(instance: _T) -> Mapper[_T]:
    ...
    Raises :class:`dis_sqlalchemy.orm.exc.UnmappedInstanceError`
    ...
    :class:`dis_sqlalchemy.exc.NoInspectionAvailable` if the instance is
...
def class_mapper(class_: Type[_O], configure: bool = True) -> Mapper[_O]:
    ...
    Using the inspection system will raise
    :class:`dis_sqlalchemy.exc.NoInspectionAvailable` if the class is not mapped.
...
class InspectionAttr:
    ...
    the dis_sqlalchemy class system can change while leaving the flags
