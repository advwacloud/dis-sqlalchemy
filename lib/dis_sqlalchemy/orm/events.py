...
class InstanceEvents(event.Events[ClassManager[Any]]):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    @classmethod
    @util.preload_module("dis_sqlalchemy.orm")
    def _accept_with(
        cls,
        target: Union[
            ClassManager[Any],
        ...
        if isinstance(target, ClassManager):
            ...
            util.warn_deprecated(
                "The `dis_sqlalchemy.orm.mapper()` symbol is deprecated and "
                "will be removed in a future release. For the mapper-wide "
                "event target, use the 'dis_sqlalchemy.orm.Mapper' class.",
                "2.0",
            )
            return ClassManager
    ...
    def _sa_event_merge_wo_load(
        ...
        that extensions like dis_sqlalchemy.ext.mutable know that object state has
    ...
    def load(self, target: _O, context: QueryContext) -> None:
        ...
        .. warning::
            ...
            * accessing attributes on a joined-inheritance subclass that
            ...
            As of dis_sqlalchemy 1.3.14, a warning is emitted when this occurs. The
...
class MapperEvents(event.Events[mapperlib.Mapper[Any]]):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    :param retval=False: when True, the user-defined event function
       ...
       * ``dis_sqlalchemy.orm.interfaces.EXT_CONTINUE`` - continue event
         processing normally.
       * ``dis_sqlalchemy.orm.interfaces.EXT_STOP`` - cancel all subsequent
    ...
    def before_mapper_configured(
        ...
        e.g.::
            ...
            from dis_sqlalchemy.orm import EXT_SKIP
...
class SessionEvents(event.Events[Session]):
    ...
    def do_orm_execute(self, orm_execute_state: ORMExecuteState) -> None:
        ...
        dis_sqlalchemy 1.4, all ORM queries that run through the
        ...
        .. note::  The :meth:`_orm.SessionEvents.do_orm_execute` event hook
           ...
           statements that are invoked by dis_sqlalchemy Core only, i.e. statements
    ...
    def after_flush(
        ...
        .. warning:: This event runs after the :class:`.Session` has emitted
           ...
           be replaced, sometimes causing confusing results.  dis_sqlalchemy will
    ...
    def after_bulk_update(self, update_context: _O) -> None:
        ...
        .. legacy:: The :meth:`_orm.SessionEvents.after_bulk_update` method
           is a legacy event hook as of dis_sqlalchemy 2.0.   The event
...
class AttributeEvents(event.Events[QueryableAttribute[Any]]):
    ...
    def init_scalar(
        ...
        dis_sqlalchemy versions did in fact change the object's state).
...
class QueryEvents(event.Events[Query[Any]]):
    ...
    .. legacy:: The :class:`_orm.QueryEvents` event methods are legacy
        as of dis_sqlalchemy 2.0, and only apply to direct use of the
