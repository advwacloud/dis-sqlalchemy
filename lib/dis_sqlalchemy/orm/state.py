...
if not TYPE_CHECKING:
    # optionally late-provided by dis_sqlalchemy.ext.asyncio.session
...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    dis_sqlalchemy ORM in order to track the state of an object;
    it is created the moment an object is instantiated, typically
    as a result of :term:`instrumentation` which dis_sqlalchemy applies
    ...
    In dis_sqlalchemy 1.0, this is only used for lazy loaders / deferred
    ...
    @util.preload_module("dis_sqlalchemy.orm.session")
    ...
    def async_session(self) -> Optional[AsyncSession]:
        ...
        This attribute is only non-None when the :mod:`dis_sqlalchemy.ext.asyncio`
