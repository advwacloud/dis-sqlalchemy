...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
        ...
        if alias is not None:
            if not isinstance(alias, str):
                ...
                util.warn_deprecated(
                    ...
                    "future release.  Please use a dis_sqlalchemy.alias() or "
                    "dis_sqlalchemy.orm.aliased() construct.",
                    version="1.4",
                )
                coerced_alias = alias
    ...
    def raiseload(self, attr: _AttrType, sql_only: bool = False) -> Self:
        ...
        raise an :exc:`~dis_sqlalchemy.exc.InvalidRequestError` upon access. The
            ...
            if isinstance(attr, str):
                if attr.startswith("." + _WILDCARD_TOKEN):
                    util.warn_deprecated(
                        ...
                        "please "
                        "comment on Issue #4390 on the dis_sqlalchemy project "
                        "tracker.",
                        version="1.4",
                    )
                    attr = attr[1:]
