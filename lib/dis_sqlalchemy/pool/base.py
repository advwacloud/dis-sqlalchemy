...
class ManagesConnection:
    ...
    is typically an adapter object provided by the dis_sqlalchemy dialect
    itself; the underlying asyncio object is available via the
    :attr:`.ManagesConnection.driver_connection` attribute.

    dis_sqlalchemy's interface for the DBAPI connection is based on the
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    def __connect(self) -> None:
        ...
        try:
            ...
            # in dis_sqlalchemy 1.4 the first_connect event is not used by
        ...
        try:
            ...
            if detach and is_gc_cleanup and dont_restore_gced:
                message = (
                    ...
                    "Please ensure that dis_sqlalchemy pooled connections are "
                    "returned to "
