...
from dis_sqlalchemy.testing import asyncio
...
# flag which indicates we are in the dis_sqlalchemy testing suite,
# and not that of Alembic or a third party dialect.
bootstrapped_as_dis_sqlalchemy = False

log = logging.getLogger("dis_sqlalchemy.testing.plugin_base")
...
def post_begin():
    ...
    from dis_sqlalchemy import testing  # noqa
    from dis_sqlalchemy.testing import fixtures, engines, exclusions  # noqa
    from dis_sqlalchemy.testing import assertions, warnings, profiling  # noqa
    from dis_sqlalchemy.testing import config, provision  # noqa
    from dis_sqlalchemy import util  # noqa
...
def _register_sqlite_numeric_dialect(opt, file_config):
    ...
    registry.register(
        ...
        "dis_sqlalchemy.dialects.sqlite.pysqlite",
...
def __ensure_cext(opt, file_config):
    if os.environ.get("REQUIRE_dis_sqlalchemy_CEXT", "0") == "1":
        from dis_sqlalchemy.util import has_compiled_ext

        try:
            has_compiled_ext(raise_=True)
        except ImportError as err:
            raise AssertionError(
                "REQUIRE_dis_sqlalchemy_CEXT is set but can't import the "
                "cython extensions"
            ) from err
...
def _setup_requirements(argument):
    ...
    config.bootstrapped_as_dis_sqlalchemy = bootstrapped_as_dis_sqlalchemy
