...
def pytest_addoption(parser):
    group = parser.getgroup("dis_sqlalchemy")
...
def _log_dis_sqlalchemy_info(session):
    import dis_sqlalchemy
    from dis_sqlalchemy import __version__
    from dis_sqlalchemy.util import has_compiled_ext
    from dis_sqlalchemy.util._has_cy import _CYEXTENSION_MSG

    greet = "dis_sqlalchemy installation"
    site = "no user site" if sys.flags.no_user_site else "user site loaded"
    msgs = [
        f"dis_sqlalchemy {__version__} ({site})",
        f"Path: {dis_sqlalchemy.__file__}",
    ]

    if has_compiled_ext():
        from dis_sqlalchemy.cyextension import util
...
def pytest_sessionstart(session):
    ...
    _log_dis_sqlalchemy_info(session)
...
def pytest_collection_finish(session):
    if session.config.option.dump_pyannotate:
        ...
        lib_dis_sqlalchemy = os.path.abspath("lib/dis_sqlalchemy")

        def _filter(filename):
            filename = os.path.normpath(os.path.abspath(filename))
            if "lib/dis_sqlalchemy" not in os.path.commonpath(
                [filename, lib_dis_sqlalchemy]
            ):
                return None
