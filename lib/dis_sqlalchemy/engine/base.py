...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    _sqla_logger_namespace = "dis_sqlalchemy.engine.Connection"

    # used by dis_sqlalchemy.engine.util.TransactionalContext
    ...
    def execution_options(self, **opt: Any) -> Connection:
        ...
        The keywords that are currently recognized by dis_sqlalchemy itself
        ...
        :param logging_token: Available on: :class:`_engine.Connection`,
          ...
          ``logging.getLogger("dis_sqlalchemy.engine")`` logger. This allows a
    ...
    def connection(self) -> PoolProxiedConnection:
        ...
        This is a dis_sqlalchemy connection-pool proxied connection
    ...
    def invalidate(self, exception: Optional[BaseException] = None) -> None:
        ...
        :class:`~dis_sqlalchemy.exc.InvalidRequestError`.
    ...
    def begin(self) -> RootTransaction:
        ...
        if self._transaction is None:
            ...
            raise exc.InvalidRequestError(
                "This connection has already initialized a dis_sqlalchemy "
    ...
    def _handle_dbapi_exception(
        ...
        try:
            ...
            if should_wrap:
                dis_sqlalchemy_exception = exc.DBAPIError.instance(
                    statement,
                    parameters,
                    cast(Exception, e),
                    self.dialect.loaded_dbapi.Error,
            ...
            if (self.dialect._has_events) and not self._execution_options.get(
                ...
                if self._is_disconnect != ctx.is_disconnect:
                    ...
                    if dis_sqlalchemy_exception:
                        dis_sqlalchemy_exception.connection_invalidated = (
                            ctx.is_disconnect
                        )
            ...
            if newraise:
                ...
                assert dis_sqlalchemy_exception is not None
                raise dis_sqlalchemy_exception.with_traceback(exc_info[2]) from e
            else:
    ...
    def _handle_dbapi_exception_noconnection(
        ...
        if dialect._has_events:
            ...
            if dis_sqlalchemy_exception and is_disconnect != ctx.is_disconnect:
                dis_sqlalchemy_exception.connection_invalidated = (
                    is_disconnect
                ) = ctx.is_disconnect
...
class ExceptionContextImpl(ExceptionContext):
    ...
    def __init__(
        ...
        dis_sqlalchemy_exception: Optional[exc.StatementError],
        ...
        self.dis_sqlalchemy_exception = dis_sqlalchemy_exception
...
class Engine(
    ...
    """
    Connects a :class:`~dis_sqlalchemy.pool.Pool` and
    :class:`~dis_sqlalchemy.engine.interfaces.Dialect` together to provide a
    source of database connectivity and behavior.

    An :class:`_engine.Engine` object is instantiated publicly using the
    :func:`~dis_sqlalchemy.create_engine` function.
    ...
    _sqla_logger_namespace: str = "dis_sqlalchemy.engine.Engine"
    ...
    @property
    def name(self) -> str:
        """String name of the :class:`~dis_sqlalchemy.engine.interfaces.Dialect`
    ...
    @property
    def driver(self) -> str:
        """Driver name of the :class:`~dis_sqlalchemy.engine.interfaces.Dialect`
        in use by this :class:`Engine`.
