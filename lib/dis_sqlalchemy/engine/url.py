...
"""Provides the :class:`~dis_sqlalchemy.engine.url.URL` class which encapsulates
information about a database connection specification.

The URL object is created automatically when
:func:`~dis_sqlalchemy.engine.create_engine` is called with a string
    ...
    * :attr:`_engine.URL.query`: an immutable mapping representing the query
    ...
    def create(
        ...
        :param drivername: the name of the database backend. This name will
          correspond to a module in dis_sqlalchemy/databases or a third party
          plug-in.
    ...
    def get_dialect(self, _is_async: bool = False) -> Type[Dialect]:
        """Return the dis_sqlalchemy :class:`_engine.Dialect` class corresponding
...
def _parse_url(name: str) -> URL:
    ...
    if m is not None:
        ...
        raise exc.ArgumentError(
            "Could not parse dis_sqlalchemy URL from string '%s'" % name
        )
