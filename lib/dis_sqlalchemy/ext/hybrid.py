...
The :mod:`~dis_sqlalchemy.ext.hybrid` extension provides a special form of
method decorator and has minimal dependencies on the rest of dis_sqlalchemy.
...
the argument, which when evaluated with dis_sqlalchemy expression mechanics
...
dis_sqlalchemy SQL expression have enough differences that two separate Python
expressions should be defined. The :mod:`~dis_sqlalchemy.ext.hybrid` decorator
...
.. note:: :attr:`.hybrid_property.inplace` as well as the use of ``@classmethod``
   for proper typing support are available as of dis_sqlalchemy 2.0.4, and will
...
dis_sqlalchemy's :class:`.hybrid_property` decorator intends that adding on these
...
is not available to dis_sqlalchemy under :pep:`484` compliance.
...
behavior of each dis_sqlalchemy expression operator individually.  They
