...
:mod:`dis_sqlalchemy.ext.mutable` extension can be used
with any type whose target Python type may be mutable, including
:class:`.PickleType`, :class:`_postgresql.ARRAY`, etc.

When using the :mod:`dis_sqlalchemy.ext.mutable` extension, the value itself
tracks all parents which reference it.  Below, we illustrate a simple
version of the :class:`.MutableDict` dictionary object, which applies
the :class:`.Mutable` mixin to a plain Python dictionary::

    from dis_sqlalchemy.ext.mutable import Mutable
...
attribute. Such as, with classical table metadata::
    ...
    from dis_sqlalchemy import Table, Column, Integer
...
The key to the :mod:`dis_sqlalchemy.ext.mutable` extension relies upon the
...
objects to each of the ``Vertex.start`` and ``Vertex.end`` attributes::
    ...
    from dis_sqlalchemy.orm import DeclarativeBase, Mapped
    from dis_sqlalchemy.orm import composite, mapped_column
...
class MutableComposite(MutableBase):
    """Mixin that defines transparent propagation of change
    events on a dis_sqlalchemy "composite" object to its
