...
r"""Define an extension to the :mod:`dis_sqlalchemy.ext.declarative` system
...
explicit table declaration::
    ...
    from dis_sqlalchemy import create_engine, MetaData, Table, Column, ForeignKey
    from dis_sqlalchemy.ext.automap import automap_base
    engine = create_engine("sqlite:///mydatabase.db")
...
``dis_sqlalchemy.ext.automap``. If no
...
``dis_sqlalchemy.ext.automap`` namespace (e.g.
``MyBase.by_module.dis_sqlalchemy.ext.automap.<classname>``), which would contain
...
The :mod:`.dis_sqlalchemy.ext.automap` extension allows classes to be defined
...
:mod:`.dis_sqlalchemy.ext.automap` is tasked with producing mapped classes and
...
options along to all one-to-many relationships::
    ...
    from dis_sqlalchemy.ext.automap import generate_relationship
...
:mod:`.dis_sqlalchemy.ext.automap` will generate many-to-many relationships, e.g.
...
:mod:`.dis_sqlalchemy.ext.automap` will not generate any relationships between
...
class AutomapBase:
    ...
    ``dis_sqlalchemy.ext.automap``; to access this namespace using
    :attr:`.AutomapBase.by_module` looks like::

        User = Base.by_module.dis_sqlalchemy.ext.automap.User
    ...
    def prepare(
        ...
        :param modulename_for_table: callable function which will be used to
         ...
         ``dis_sqlalchemy.ext.automap`` for these classes.
