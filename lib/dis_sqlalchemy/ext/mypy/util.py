...
def _set_info_metadata(info: TypeInfo, key: str, data: Any) -> None:
    info.metadata.setdefault("dis_sqlalchemy", {})[key] = data


def _get_info_metadata(info: TypeInfo, key: str) -> Optional[Any]:
    return info.metadata.get("dis_sqlalchemy", {}).get(key, None)
...
def establish_as_dis_sqlalchemy(info: TypeInfo) -> None:
    info.metadata.setdefault("dis_sqlalchemy", {})
...
def get_mapped_attributes(
...
) -> Optional[List[dis_sqlalchemyAttribute]]:
    ...
    attributes: List[dis_sqlalchemyAttribute] = []

    for data in mapped_attributes:
        attr = dis_sqlalchemyAttribute.deserialize(info, data, api)
...
def set_mapped_attributes(
    info: TypeInfo, attributes: List[dis_sqlalchemyAttribute]
) -> None:
    _set_info_metadata(
        info,
        "mapped_attributes",
...
def fail(api: SemanticAnalyzerPluginInterface, msg: str, ctx: Context) -> None:
    msg = "[dis_sqlalchemy Mypy plugin] %s" % msg
