...
def apply_mypy_mapped_attr(
    ...
    attributes: List[util.dis_sqlalchemyAttribute],
) -> None:
        ...
        if (
            ...
            if (
                ...
                if new_python_type_for_type is not None and not isinstance(
                    ...
                    # update the dis_sqlalchemyAttribute with the better
                    # information
                    mapped_attr_lookup[
                        stmt.lvalues[0].name
...
def add_additional_orm_attributes(
    ...
    if "__init__" not in info.names and not is_base:
        ...
        for base in info.mro[1:-1]:
            if "dis_sqlalchemy" not in info.metadata:
                continue
    ...
    if "__table__" not in info.names and util.get_has_table(info):
        _apply_placeholder_attr_to_class(
            api, cls, "dis_sqlalchemy.sql.schema.Table", "__table__"
        )
    if not is_base:
        _apply_placeholder_attr_to_class(
            api, cls, "dis_sqlalchemy.orm.mapper.Mapper", "__mapper__"
        )
