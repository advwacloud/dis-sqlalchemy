...
def _infer_type_from_mapped(
    ...
    # NameExpr(query_expression [dis_sqlalchemy.orm._orm_constructors.query_expression])  # noqa: E501
    # (Pdb) stmt.rvalue.callee.node
    # <mypy.nodes.FuncDef object at 0x7f8d92fb5940>
    # (Pdb) stmt.rvalue.callee.node.type
    # def [_T] (default_expr: dis_sqlalchemy.sql.elements.ColumnElement[_T`-1] =) -> dis_sqlalchemy.orm.base.Mapped[_T`-1]  # noqa: E501
    # dis_sqlalchemy.orm.base.Mapped[_T`-1]
...
def extract_python_type_from_typeengine(
    ...
    if node.fullname == "dis_sqlalchemy.sql.sqltypes.Enum" and type_args:
    ...
    assert node.has_base("dis_sqlalchemy.sql.type_api.TypeEngine"), (
        "could not extract Python type from node: %s" % node
    )

    type_engine_sym = api.lookup_fully_qualified_or_none(
        "dis_sqlalchemy.sql.type_api.TypeEngine"
    )
