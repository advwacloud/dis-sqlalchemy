# ext/mypy/plugin.py
# Copyright (C) 2021-2023 the dis_sqlalchemy authors and contributors
# <see AUTHORS file>
#
...
"""
Mypy plugin for dis_sqlalchemy ORM.
...
try:
    __import__("dis_sqlalchemy-stubs")
except ImportError:
    pass
else:
    raise ImportError(
        "The dis_sqlalchemy mypy plugin in dis_sqlalchemy "
        "2.0 does not work with dis_sqlalchemy-stubs or "
        "dis_sqlalchemy2-stubs installed, as well as with any other third party "
        "dis_sqlalchemy stubs.  Please uninstall all dis_sqlalchemy stubs "
        "packages."
    )


class dis_sqlalchemyPlugin(Plugin):
    ...
    def get_attribute_hook(
        ...
        if fullname.startswith(
            "dis_sqlalchemy.orm.attributes.QueryableAttribute."
        ):
    ...
    def get_additional_deps(
        ...
        return [
            #
            (10, "dis_sqlalchemy.orm", -1),
            (10, "dis_sqlalchemy.orm.attributes", -1),
            (10, "dis_sqlalchemy.orm.decl_api", -1),
        ]


def plugin(version: str) -> TypingType[dis_sqlalchemyPlugin]:
    return dis_sqlalchemyPlugin
...
def _add_globals(ctx: Union[ClassDefContext, DynamicClassDefContext]) -> None:
    ...
    util.add_global(ctx, "dis_sqlalchemy.orm", "Mapped", "__sa_Mapped")
