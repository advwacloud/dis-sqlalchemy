...
def literal(
    ...
    subclass,
    such as a :class:`~dis_sqlalchemy.schema.Column` object.  Use this function
    ...
    :param type\_: an optional :class:`~dis_sqlalchemy.types.TypeEngine` which will
...
def literal_column(
    ...
    :param type\_: an optional :class:`~dis_sqlalchemy.types.TypeEngine`
      object which will
...
class CompilerElement(Visitable):
    ...
    @util.preload_module("dis_sqlalchemy.engine.default")
    @util.preload_module("dis_sqlalchemy.engine.url")
    def compile(
        self,
...
class ClauseElement(
    ...
    def self_group(
        ...
        need to use this method directly.  Note that dis_sqlalchemy's
...
class ColumnElement(
    ...
    A wide variety of dis_sqlalchemy Core functions work at the SQL expression
    level, and are intended to accept instances of
    :class:`_expression.ColumnElement` as
    arguments.  These functions will typically document that they accept a
    "SQL expression" as an argument.  What this means in terms of dis_sqlalchemy
    usually refers to an input which is either already in the form of a
    :class:`_expression.ColumnElement` object,
    or a value which can be **coerced** into
    one.  The coercion rules followed by most, but not all, dis_sqlalchemy Core
    functions with regards to SQL expressions are as follows:
        ...
        * a literal Python value, such as a string, integer or floating
          ...
          ``execute()`` or ``executemany()`` methods, after dis_sqlalchemy
        ...
        * The Python ``None`` value is typically interpreted as ``NULL``,
          which in dis_sqlalchemy Core produces an instance of :func:`.null`.
    ...
    .. sourcecode:: pycon+sql
        ...
        >>> column('a') + column('b')
        <dis_sqlalchemy.sql.expression.BinaryExpression object at 0x101029dd0>
    ...
    def _non_anon_label(self) -> Optional[str]:
        ...
        object, as has been the precedent established in dis_sqlalchemy for many
...
class TextClause(
    ...
    @util.preload_module("dis_sqlalchemy.sql.selectable")
    def columns(
        ...
        of a "selectable"::
            ...
            from dis_sqlalchemy.sql import column, text
        ...
        the effective return type of the column, so that dis_sqlalchemy's
...
class Cast(WrapsColumnExpression[_T]):
    ...
    as in::
        ...
        from dis_sqlalchemy import cast, Numeric
...
class FunctionFilter(ColumnElement[_T]):
    ...
    def over(
        ...
        is shorthand for::
            ...
            from dis_sqlalchemy import over, funcfilter
...
class quoted_name(util.MemoizedSlots, str):
    ...
    as uppercase.  The dis_sqlalchemy dialects for these backends
    convert from dis_sqlalchemy's lower-case-means-insensitive convention
    ...
    .. versionchanged:: 1.2 The :class:`.quoted_name` construct is now
       importable from ``dis_sqlalchemy.sql``, in addition to the previous
       location of ``dis_sqlalchemy.sql.elements``.
