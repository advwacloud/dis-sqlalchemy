...
class DDLEvents(event.Events[SchemaEventTarget]):
    ...
    **Create / Drop Events**
    ...
    after ``CREATE TABLE`` is emitted::
        ...
        from dis_sqlalchemy import Table, Column, Metadata, Integer
    ...
    for ``CREATE INDEX``, however dis_sqlalchemy does not normally emit
    ...
    .. note:: These event hooks are only emitted within the scope of
       dis_sqlalchemy's create/drop methods; they are not necessarily supported
       by tools such as `alembic <https://alembic.dis_sqlalchemy.org>`_.
    ...
    def column_reflect(
        ...
        by :meth:`.reflection.Inspector.get_columns`:
            ...
            * ``type`` - the type of this column, which should be an instance
              of :class:`~dis_sqlalchemy.types.TypeEngine`, is applied to the
