    ...
    def __init__(
        ...
        :param collation: Optional, a column-level collation for
          ...
          .. sourcecode:: pycon+sql
            ...
            >>> from dis_sqlalchemy import cast, select, String
...
class Unicode(String):
    ...
    In modern dis_sqlalchemy, use of the :class:`.Unicode` datatype does not
    imply any encoding/decoding behavior within dis_sqlalchemy itself.  In Python
    3, all string objects are inherently Unicode capable, and dis_sqlalchemy
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    .. note::
        ...
        the dis_sqlalchemy dialect in use.   Users requiring specific precision/
    ...
    def __init__(
        ...
        :param precision: the numeric precision for use in DDL ``CREATE
           ...
           .. note:: For the Oracle backend, the
              ...
              in version 2.0 of dis_sqlalchemy.
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def __init__(self, *enums: object, **kw: Any):
        ...
        :param sort_key_function: a Python callable which may be used as the
           "key" argument in the Python ``sorted()`` built-in.   The dis_sqlalchemy
    ...
    def _resolve_for_python_type(
        ...
        # as ``dis_sqlalchemy.Enum(enum.Enum)`` which indicates we need to
    ...
    @util.preload_module("dis_sqlalchemy.sql.schema")
    def _set_table(self, column, table):
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    :attr:`~dis_sqlalchemy.sql.expression.func`).
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    .. note::  :class:`_types.JSON`
       ...
       * PostgreSQL - see :class:`dis_sqlalchemy.dialects.postgresql.JSON` and
         :class:`dis_sqlalchemy.dialects.postgresql.JSONB` for backend-specific
         notes

       * MySQL - see
         :class:`dis_sqlalchemy.dialects.mysql.JSON` for backend-specific notes

       * SQLite as of version 3.9 - see
         :class:`dis_sqlalchemy.dialects.sqlite.JSON` for backend-specific notes

       * Microsoft SQL Server 2016 and later - see
         :class:`dis_sqlalchemy.dialects.mssql.JSON` for backend-specific notes
    ...
    * Data casters for specific JSON element types, subsequent to an index
    ...
    :class:`dis_sqlalchemy.dialects.postgresql.JSON` and
    :class:`dis_sqlalchemy.dialects.postgresql.JSONB` which both offer additional
    ...
    * :meth:`.JSON.Comparator.as_integer` - return the element as an integer
    ...
    .. note::
        ...
        this looked like::
           ...
           from dis_sqlalchemy import cast, type_coerce
           from dis_sqlalchemy import String, JSON
           cast(
        ...
        series, see the documentation for dis_sqlalchemy 1.2 or the included HTML
        files in the doc/ directory of the version's distribution.

    **Detecting Changes in JSON columns when using the ORM**

    The :class:`_types.JSON` type, when used with the dis_sqlalchemy ORM, does not
    detect in-place mutations to the structure.  In order to detect these, the
    :mod:`dis_sqlalchemy.ext.mutable` extension must be used, most typically
    ...
    **Customizing the JSON Serializer**
    ...
    .. seealso::
        ...
        :class:`dis_sqlalchemy.dialects.postgresql.JSON`

        :class:`dis_sqlalchemy.dialects.postgresql.JSONB`

        :class:`dis_sqlalchemy.dialects.mysql.JSON`

        :class:`dis_sqlalchemy.dialects.sqlite.JSON`
...
class ARRAY(
    ...
    .. note::  This type serves as the basis for all ARRAY operations.
       ...
       arrays in dis_sqlalchemy**. It is recommended to use the PostgreSQL-specific
       :class:`dis_sqlalchemy.dialects.postgresql.ARRAY` type directly when using
    ...
    some third-party dialects, no other dis_sqlalchemy built-in dialect has support
    for this type.
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    .. container:: topic
        ...
        **Detecting Changes in ARRAY columns when using the ORM**
        ...
        The :class:`_sqltypes.ARRAY` type, when used with the dis_sqlalchemy ORM,
        does not detect in-place mutations to the array. In order to detect
        these, the :mod:`dis_sqlalchemy.ext.mutable` extension must be used, using
    ...
    .. seealso::
        ...
        :class:`dis_sqlalchemy.dialects.postgresql.ARRAY`
    ...
    class Comparator(
        ...
        @util.preload_module("dis_sqlalchemy.sql.elements")
        def any(self, other, operator=None):
            ...
            :param operator: an operator object from the
             :mod:`dis_sqlalchemy.sql.operators`
...
class MatchType(Boolean):
    ...
    operator in generic dis_sqlalchemy Core, we can't assume the return type
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    ...
    **Python uuid objects**, from the Python
    ...
    module::
        ...
        from dis_sqlalchemy import Table, Column, MetaData, String
...
def _resolve_value_to_type(value: Any) -> TypeEngine[Any]:
    ...
    if _result_type is False:
        # use inspect() to detect dis_sqlalchemy built-in
        # objects.
