...
def listen(
    ...
    dis_sqlalchemy event system, documented at :ref:`event_toplevel`.

    e.g.::

        from dis_sqlalchemy import event
        from dis_sqlalchemy.schema import UniqueConstraint
    ...
    :param bool insert: The default behavior for event handlers is to append
      ...
      ``insert=True``, dis_sqlalchemy will insert (prepend) the function to the
      internal list upon discovery. This feature is not typically used or
      recommended by the dis_sqlalchemy maintainers, but is provided to ensure
