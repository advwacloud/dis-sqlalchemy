...
"""Exceptions used with dis_sqlalchemy.

The base exception class is :exc:`.dis_sqlalchemyError`.  Exceptions which are
...
class dis_sqlalchemyError(HasDescriptionCode, Exception):
    """Generic error class."""
    ...
    def _message(self) -> str:
        ...
        # dis_sqlalchemy though this is happening in at least one known external
        ...
        if len(self.args) == 1:
            ...
            # this is not a normal case within dis_sqlalchemy but is here for
...
class ArgumentError(dis_sqlalchemyError):
...
class CircularDependencyError(dis_sqlalchemyError):
    ...
    * In a :attr:`_schema.MetaData.sorted_tables` operation, two
    ...
    def __init__(
        ...
        if msg is None:
            ...
            message = msg
        dis_sqlalchemyError.__init__(self, message, code=code)
        self.cycles = cycles
        self.edges = edges
...
class CompileError(dis_sqlalchemyError):
...
class IdentifierError(dis_sqlalchemyError):
    """Raised when a schema name is beyond the max character limit"""


class DisconnectionError(dis_sqlalchemyError):
    ...
    the pool gives up and raises :class:`~dis_sqlalchemy.exc.InvalidRequestError`
...
class TimeoutError(dis_sqlalchemyError):  # noqa
    """Raised when a connection pool times out on getting a connection."""


class InvalidRequestError(dis_sqlalchemyError):
    """dis_sqlalchemy was asked to do something it can't do.
...
class NoInspectionAvailable(InvalidRequestError):
    """A subject passed to :func:`dis_sqlalchemy.inspection.inspect` produced
...
class NoResultFound(InvalidRequestError):
    ...
    .. versionchanged:: 1.4  This exception is now part of the
       ``dis_sqlalchemy.exc`` module in Core, moved from the ORM.  The symbol
       remains importable from ``dis_sqlalchemy.orm.exc``.
...
class DontWrapMixin:
    ...
    E.g.::
        ...
        from dis_sqlalchemy.exc import DontWrapMixin
...
class StatementError(dis_sqlalchemyError):
    ...
    @_preloaded.preload_module("dis_sqlalchemy.sql.util")
    def _sql_message(self) -> str:
...
class DBAPIError(StatementError):
    ...
    DB-API exception types are wrapped by matching sub-types of dis_sqlalchemy's
    :class:`DBAPIError` when possible.  DB-API's ``Error`` type maps to
    :class:`DBAPIError` in dis_sqlalchemy, otherwise the names are identical.  Note
    ...
    def instance(
        ...
        if orig is not None:
            ...
            if isinstance(orig, dis_sqlalchemyError) and statement:
...
class Base20DeprecationWarning(SADeprecationWarning):
    ...
    def __str__(self) -> str:
        return (
            super().__str__()
            + " (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)"
        )
...
class SAPendingDeprecationWarning(PendingDeprecationWarning):
    ...
    is not used in modern versions of dis_sqlalchemy.
