...
def decorator(target: Callable[..., Any]) -> Callable[[_Fn], _Fn]:
    ...
    def decorate(fn: _Fn) -> _Fn:
        ...
        # dis_sqlalchemy that arguments should be passed positionally
...
class classproperty(property):
    ...
    :class:`~.dis_sqlalchemy.ext.declarative.declared_attr`
...
_not_sa_pattern = re.compile(r"^(?:dis_sqlalchemy\.(?!testing)|alembic\.)")
...
def _warnings_warn(
    ...
    # adjust the given stacklevel to be outside of dis_sqlalchemy
    try:
...
_SQLA_RE = re.compile(r"dis_sqlalchemy/([a-z_]+/){0,2}[a-z_]+\.py")
