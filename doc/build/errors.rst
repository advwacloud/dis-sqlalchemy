...
and warnings raised or emitted by dis_sqlalchemy.

dis_sqlalchemy normally raises errors within the context of a dis_sqlalchemy-specific
exception class.  For details on these classes, see
:ref:`core_exceptions_toplevel` and :ref:`orm_exceptions_toplevel`.

dis_sqlalchemy errors can roughly be separated into two categories, the
...
which typically applies to nearly all dis_sqlalchemy applications.

The following points summarize what this error means, beginning with the
most fundamental points that most dis_sqlalchemy users should already be
familiar with.

* **The dis_sqlalchemy Engine object uses a pool of connections by default** - What
...
in the 2.x series of dis_sqlalchemy when any SQL statements are emitted.  When a connection is invalidated, any :class:`_engine.Transaction`
...
dis_sqlalchemy does not generate these exceptions directly.  Instead, they are
intercepted from the database driver and wrapped by the dis_sqlalchemy-provided
exception :class:`.DBAPIError`, however the messaging within the exception is
**generated by the driver, not dis_sqlalchemy**.
...
the database driver (DBAPI), not dis_sqlalchemy itself.
...
effectively produce a **performance degradation** compared to prior dis_sqlalchemy
...
For the above two reasons, dis_sqlalchemy's caching system is **extremely
...
* Third party or user defined SQL constructs that subclass from classes such
  ...
  work with the :ref:`dis_sqlalchemy.ext.compiler_toplevel`, should normally
...
``dis_sqlalchemy.dialects.postgresql.PGCompiler``.  The guidance below is
...
`"insert on conflict" <postgresql_insert_on_conflict>`_ construct::
  ...
  dis_sqlalchemy.exc.UnsupportedCompilationError:
  Compiler <dis_sqlalchemy.sql.compiler.StrSQLCompiler object at 0x7f04fc17e320>
  can't render element of type
  <class 'dis_sqlalchemy.dialects.postgresql.dml.OnConflictDoNothing'>
...
.. code-block::
 ...
 dis_sqlalchemy.exc.StatementError: (dis_sqlalchemy.exc.InvalidRequestError)
...
This refers to a change made as of dis_sqlalchemy 1.4 where a SELECT statement as generated
...
In previous versions of dis_sqlalchemy, using a SELECT inside of another SELECT
...
Beyond the above practical reasons, there are a lot of other dis_sqlalchemy-oriented
...
alias to one side or the other; dis_sqlalchemy applies an alias to the right side
...
means it must be aliased.   When we create such a join using the dis_sqlalchemy
...
When dis_sqlalchemy loads ORM rows that each contain an ``Employee`` and a
...
dis_sqlalchemy 2.0 introduced a new system described at :ref:`change_7433`, which
...
the proactive warning system dis_sqlalchemy implements would still otherwise produce
...
the scope of the ``B.a`` relationship::
    ...
    >>> b1.a = a1
    >>> b2.a = a1
    dis_sqlalchemy.exc.InvalidRequestError: Instance <A at 0x7eff44359350> is
...
dis_sqlalchemy 2.0 introduces a new
...
:class:`_orm.Mapped` to be properly annotated. Legacy dis_sqlalchemy mappings which
...
This warning occurs when using the dis_sqlalchemy ORM Mapped Dataclasses feature
...
.. sourcecode:: none
    ...
    will raise an error in dis_sqlalchemy 2.1. When declaring dis_sqlalchemy
...
dis_sqlalchemy's dataclasses feature within the 2.0 series does not honor this
...
Since their presence is ambiguous otherwise, dis_sqlalchemy 2.1 will require that
mixin classes which have dis_sqlalchemy mapped attributes within a dataclass
...
:meth:`_orm.registry.mapped_as_dataclass` decorator, dis_sqlalchemy makes use
...
use of parameters such as ``init=False``, ``kw_only=True``, etc.   **dis_sqlalchemy
...
.. seealso::
  ...
  :ref:`orm_declarative_native_dataclasses` - dis_sqlalchemy dataclasses documentation
...
The dis_sqlalchemy async mode requires an async driver to be used to connect to the db.
This error is usually raised when trying to use the async version of dis_sqlalchemy
...
context usually setup by the dis_sqlalchemy AsyncIO proxy classes. Usually this
...
Exceptions in this section are not generated by current dis_sqlalchemy
versions, however are provided here to suit exception message hyperlinks.

.. _error_b8d9:

The <some function> in dis_sqlalchemy 2.0 will no longer <something>
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

dis_sqlalchemy 2.0 represents a major shift for a wide variety of key
...
fundamental assumptions of dis_sqlalchemy since its early beginnings, and
...
Introduced at :ref:`migration_20_toplevel`, the dis_sqlalchemy 2.0 project includes
a comprehensive future compatibility system that's integrated into the
1.4 series of dis_sqlalchemy, such that applications will have a clear,
...
.. seealso::
    ...
    the 1.x series, as well as the current goals and progress of dis_sqlalchemy
    2.0.


    :ref:`deprecation_20_mode` - specific guidelines on how to use
    "2.0 deprecations mode" in dis_sqlalchemy 1.4.
...
This message refers to the "backref cascade" behavior of dis_sqlalchemy,
...
be set to ``False`` to disable it, and in dis_sqlalchemy 2.0 the "cascade backrefs"
behavior has been removed entirely.

For older dis_sqlalchemy versions, to set
...
.. seealso::
    ...
    :ref:`change_5150` - background on the change for dis_sqlalchemy 2.0.
...
The :func:`_expression.select` construct has been updated as of dis_sqlalchemy
1.4 to support the newer calling style that is standard in
dis_sqlalchemy 2.0.   For backwards compatibility within
...
For comparison, a :func:`_expression.select` in legacy forms of dis_sqlalchemy,
before methods like :meth:`.Select.where` were even added, would like::

    # this is how it was documented in original dis_sqlalchemy versions
    # many years ago
...
Or even that the "whereclause" would be passed positionally::
    ...
    # this is also how it was documented in original dis_sqlalchemy versions
    # many years ago
...
The concept of "bound metadata" is present up until dis_sqlalchemy 1.4; as
of dis_sqlalchemy 2.0 it's been removed.
...
:class:`_orm.Engine`. In dis_sqlalchemy 2.0, the :class:`_orm.Session` must be
...
In dis_sqlalchemy 1.4, this :term:`2.0 style` behavior is enabled when the
...
dis_sqlalchemy pattern present only in 1.x versions. The issue occurs when one invokes
...
Note that the concept of bound metadata is **not present in dis_sqlalchemy 2.0**.
...
This error condition was added to dis_sqlalchemy as of version 1.4, and does not
apply to dis_sqlalchemy 2.0.    The error
...
The "subtransaction" pattern is removed in dis_sqlalchemy 2.0 so that this
