...
dis_sqlalchemy supports three forms of inheritance: **single table inheritance**,
...
When mappers are configured in an inheritance relationship, dis_sqlalchemy has the
...
.. topic:: Joined inheritance primary keys
    ...
    the base table alone. This has obvious advantages, so dis_sqlalchemy always
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.ArgumentError: Column 'start_date' on class Manager conflicts
...
constructed using a dis_sqlalchemy helper :func:`.polymorphic_union`.
...
Above, we are not actually making use of dis_sqlalchemy's inheritance mapping
...
class called :class:`.AbstractConcreteBase` which achieves this automatically::
    ...
    from dis_sqlalchemy.ext.declarative import AbstractConcreteBase
    from dis_sqlalchemy.orm import DeclarativeBase
...
themselves refer back to ``Company``.   For this case, dis_sqlalchemy has
...
in current dis_sqlalchemy versions.   Normally, the :func:`_orm.with_polymorphic`
