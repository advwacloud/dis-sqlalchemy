...
:class:`~dis_sqlalchemy.orm.session.Session` within its transactional context.
This is most easily accomplished using the
:meth:`~.Session.execute` method, which returns a
:class:`~dis_sqlalchemy.engine.CursorResult` in the same manner as an
:class:`~dis_sqlalchemy.engine.Engine` or
:class:`~dis_sqlalchemy.engine.Connection`::
...
The current :class:`~dis_sqlalchemy.engine.Connection` held by the
:class:`~dis_sqlalchemy.orm.session.Session` is accessible using the
...
behavior of dis_sqlalchemy's insert behavior with both client-side and server-side
defaults::
...
dis_sqlalchemy Core supports a method of retrieving these primary key values which
...
A future release of dis_sqlalchemy may seek to improve the efficiency of
...
is not supported, and where RETURNING is supported dis_sqlalchemy will use that.
...
dis_sqlalchemy renders an INSERT for the "data" field, but only includes "id" in
...
Where above, dis_sqlalchemy renders ``my_sequence.nextval`` for the primary key
...
make sure dis_sqlalchemy can "pre-execute" the default, which means it has to be an
...
into the column::
    ...
    from dis_sqlalchemy import cast, Binary
...
dis_sqlalchemy also supports non-DDL server side defaults, as documented at
...
defaults and are actively rendered by dis_sqlalchemy itself. This limitation may be
addressed in future dis_sqlalchemy releases.
...
dis_sqlalchemy 2.0 includes enhanced capabilities for emitting several varieties
...
Due to limited support within DBAPIs,  dis_sqlalchemy has limited support for two-
...
::
    ...
    from dis_sqlalchemy.sql import Update, Delete
    from dis_sqlalchemy.orm import Session, sessionmaker
    import random
...
.. seealso::
    ...
    `Django-style Database Routers in dis_sqlalchemy <https://techspot.zzzeek.org/2012/01/11/django-style-database-routers-in-dis_sqlalchemy/>`_  - blog post on a more comprehensive example of :meth:`.Session.get_bind`

Horizontal Partitioning
-----------------------

Horizontal partitioning partitions the rows of a single table (or a set of
tables) across multiple databases.    The dis_sqlalchemy :class:`.Session`
...
.. legacy::
  ...
  dis_sqlalchemy 2.0 has integrated the :class:`_orm.Session` "bulk insert" and
