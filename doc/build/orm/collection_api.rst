...
object directly::
    ...
    from dis_sqlalchemy.orm import column_keyed_dict
...
with a ``@property`` as mentioned earlier::
    ...
    from dis_sqlalchemy.orm import mapped_collection
...
In other cases, special decorators are needed to tell dis_sqlalchemy more detail
...
Collections in dis_sqlalchemy are transparently *instrumented*. Instrumentation
...
parent's :class:`~dis_sqlalchemy.orm.session.Session` (i.e. the ``save-update``
...
use by dis_sqlalchemy. The ORM needs to know which methods to use to append, remove
...
And that's all that's needed to complete the example. dis_sqlalchemy will add
...
interface marked for dis_sqlalchemy's use. Append and remove methods will be
...
rare cases::
    ...
    from dis_sqlalchemy.orm.collections import KeyFuncDict, collection
...
methods in the basic dictionary interface for dis_sqlalchemy to use by default.
...
.. autoclass:: dis_sqlalchemy.orm.KeyFuncDict
   :members:

.. autodata:: dis_sqlalchemy.orm.MappedCollection


Collection Internals
-----------------------------

.. currentmodule:: dis_sqlalchemy.orm.collections
