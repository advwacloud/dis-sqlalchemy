...
.. sidebar::  ``mapped_column()`` supersedes the use of ``Column()``
  ...
  Users of 1.x dis_sqlalchemy will note the use of the :func:`_orm.mapped_column`
  construct, which is new as of the dis_sqlalchemy 2.0 series.  This ORM-specific
  ...
  annotation-driven configuration style introduced in dis_sqlalchemy 2.0.
...
if used,  **must**
be present within a special dis_sqlalchemy type called :class:`_orm.Mapped`, which
...
* **datatype** - the Python type given inside :class:`_orm.Mapped`, as contained
  ...
  dis_sqlalchemy datatype.   This dictionary is completely customizable,
...
* **nullability** - The :func:`_orm.mapped_column` construct will indicate
  ...
  :paramref:`_orm.mapped_column.primary_key` parameter, then dis_sqlalchemy's usual
...
The mapping of Python types to dis_sqlalchemy :class:`_types.TypeEngine` types
described in the previous section defaults to a hardcoded dictionary
present in the ``dis_sqlalchemy.sql.sqltypes`` module.  However, the :class:`_orm.registry`
...
the registry and Declarative base could be configured as::
    ...
    from dis_sqlalchemy import BIGINT, Integer, NVARCHAR, String, TIMESTAMP
    from dis_sqlalchemy.orm import DeclarativeBase
    from dis_sqlalchemy.orm import Mapped, mapped_column, registry
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy.dialects import mssql, postgresql
...
apply to dis_sqlalchemy and searching only for dis_sqlalchemy ORM constructs.
...
class are automatically linked to the dis_sqlalchemy :class:`.Enum` datatype
...
dis_sqlalchemy :class:`.Enum` SQL type, using a special form which indicates to the
...
would be indicated explicitly as::
    ...
    class Base(DeclarativeBase):
        type_annotation_map = {
            enum.Enum: dis_sqlalchemy.Enum(enum.Enum),
            typing.Literal: dis_sqlalchemy.Enum(enum.Enum),
        }
...
type map::
    ...
    class Base(DeclarativeBase):
        type_annotation_map = {
            Status: dis_sqlalchemy.Enum("pending", "received", "completed", name="status_enum"),
        }
...
Or alternatively within :func:`_orm.mapped_column`::
    ...
    class SomeClass(Base):
        ...
        status: Mapped[Status] = mapped_column(
            dis_sqlalchemy.Enum("pending", "received", "completed", name="status_enum")
        )
...
:paramref:`.Enum.native_enum` parameter may be set to False for all types::
    ...
    class Base(DeclarativeBase):
        type_annotation_map = {
            enum.Enum: dis_sqlalchemy.Enum(enum.Enum, native_enum=False),
            typing.Literal: dis_sqlalchemy.Enum(enum.Enum, native_enum=False),
        }
...
datatype::
    ...
    class Base(DeclarativeBase):
        type_annotation_map = {
            Status: dis_sqlalchemy.Enum(Status, length=50, native_enum=False)
        }
...
The :func:`_orm.mapped_column` construct integrates with dis_sqlalchemy's
