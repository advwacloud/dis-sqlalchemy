...
For readers new to the dis_sqlalchemy ORM and/or new to Python in general,
...
dis_sqlalchemy features two distinct styles of mapper configuration, which then
...
In modern dis_sqlalchemy, the difference between these styles is mostly
superficial; when a particular dis_sqlalchemy configurational style is used to
...
dis_sqlalchemy now refers to these two mapping styles as **imperative mapping**
and **declarative mapping**.

Regardless of what style of mapping used, all ORM mappings as of dis_sqlalchemy 1.4
...
modern dis_sqlalchemy. The most common pattern is to first construct a base class
...
the use of a declarative base which is then used in a declarative table mapping::
    ...
    from dis_sqlalchemy import Integer, String, ForeignKey
...
base class (within dis_sqlalchemy's documentation it's typically referred towards
...
.. tip:: The imperative mapping form is a lesser-used form of mapping that
   originates from the very first releases of dis_sqlalchemy in 2006.  It's
...
.. versionchanged:: 2.0  The :meth:`_orm.registry.map_imperatively` method
   is now used to create classical mappings.  The ``dis_sqlalchemy.orm.mapper()``
...
shared for all mapped classes that are related to each other::
    ...
    from dis_sqlalchemy import Table, Column, Integer, String, ForeignKey
...
Detailed information is available including :attr:`_orm.Mapper.columns`::
    ...
    >>> insp.columns
    <dis_sqlalchemy.util._collections.OrderedProperties object at 0x102f407f8>
...
attributes as well as hybrids, association proxies::
    ...
    <dis_sqlalchemy.util._collections.ImmutableProperties object at 0x1040e2c68>
...
object::
  ...
  >>> insp = inspect(u1)
  >>> insp
  <dis_sqlalchemy.orm.state.InstanceState object at 0x7f07e5fec2e0>
...
The :class:`_orm.Session` to which the object is :term:`attached`, if any::
  ...
  >>> insp.session
  <dis_sqlalchemy.orm.session.Session object at 0x7f07e614f160>
...
.. [2] There is a legacy feature known as a "non primary mapper", where
       ...
       to the class.  This feature is deprecated as of dis_sqlalchemy 1.3.
