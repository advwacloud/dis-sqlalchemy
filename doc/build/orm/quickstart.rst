...
may be indicated using dis_sqlalchemy type objects in the right side
...
that are accepted by the dis_sqlalchemy :class:`_schema.Column` class, which is
used by dis_sqlalchemy Core to represent database columns.
...
of column declarations is referred towards in dis_sqlalchemy as :term:`table metadata`.
...
.. sourcecode:: pycon+sql
    ...
    [...] ('spongebob@dis_sqlalchemy.org', 1)
    INSERT INTO address (email_address, user_id) VALUES (?, ?) RETURNING id
    [...] ('sandy@dis_sqlalchemy.org', 2)
    INSERT INTO address (email_address, user_id) VALUES (?, ?) RETURNING id
...
method that's part of all dis_sqlalchemy column-like constructs to use the
SQL IN operator.
...
.. sourcecode:: pycon+sql
    ...
    ...     .where(Address.email_address == "sandy@dis_sqlalchemy.org")
    ... )
    >>> sandy_address = session.scalars(stmt).one()
    {execsql}SELECT address.id, address.email_address, address.user_id
    FROM address JOIN user_account ON user_account.id = address.user_id
    WHERE user_account.name = ? AND address.email_address = ?
    [...] ('sandy', 'sandy@dis_sqlalchemy.org')
    {stop}
    >>> sandy_address
    Address(id=2, email_address='sandy@dis_sqlalchemy.org')

The above query illustrates multiple WHERE criteria which are automatically
chained together using AND, as well as how to use dis_sqlalchemy column-like
...
.. sourcecode:: pycon+sql
    ...
    >>> patrick.addresses.append(Address(email_address="patrickstar@dis_sqlalchemy.org"))
    ...
    >>> sandy_address.email_address = "sandy_cheeks@dis_sqlalchemy.org"

    >>> session.commit()
    {execsql}UPDATE address SET email_address=? WHERE address.id = ?
    [...] ('sandy_cheeks@dis_sqlalchemy.org', 2)
    INSERT INTO address (email_address, user_id) VALUES (?, ?)
    [...] ('patrickstar@dis_sqlalchemy.org', 3)
    COMMIT
    {stop}
