...
As dis_sqlalchemy has evolved, different ORM configurational styles have emerged.
...
which is the most modern form of dis_sqlalchemy Declarative mapping::
...
is dis_sqlalchemy's original mapping form before Declarative was made (which
...
with which to link::
    ...
    # note for a Core table, we use the dis_sqlalchemy.Column construct,
    # not dis_sqlalchemy.orm.mapped_column
...
.. tip::
    ...
    It is also **recommended**, though not in any way required by dis_sqlalchemy,
...
* If there is a :func:`_orm.relationship` from ``Parent`` to ``Child``, but there is
  **not** a reverse-relationship that links a particular ``Child`` to each ``Parent``,
  dis_sqlalchemy will not have any awareness that when deleting this particular
...
* If there is a relationship that links a particular ``Child`` to each ``Parent``,
  suppose it's called ``Child.parents``, dis_sqlalchemy by default will load in
  ...
  this relationship does not need to be bidirectional; dis_sqlalchemy is strictly
...
* A higher performing option here is to use ON DELETE CASCADE directives
  ...
  "secondary" table as referencing rows in "child" are deleted.   dis_sqlalchemy
...
access to the ``Association`` object is optional, dis_sqlalchemy
...
for this declarative base, as well as the contents of the ``dis_sqlalchemy``
