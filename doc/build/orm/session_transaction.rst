...
In modern versions of dis_sqlalchemy, when a SAVEPOINT initiated by
...
methods.   Using dis_sqlalchemy 2.0-style operation, these methods affect the
...
will always commit the **outermost** transaction; this is a dis_sqlalchemy 2.0
...
interacting with transactions not managed by dis_sqlalchemy. To use two phase
...
**the DBAPI does not use a transaction under any circumstances**.  dis_sqlalchemy
methods like ``.begin()``, ``.commit()`` and ``.rollback()`` pass silently.

dis_sqlalchemy's dialects support settable isolation modes on a per-:class:`_engine.Engine`
...
.. seealso::
    ...
    the level of the dis_sqlalchemy :class:`_engine.Connection` object as well.
...
in progress, and some DBAPIs and dis_sqlalchemy dialects
...
The above recipe is part of dis_sqlalchemy's own CI to ensure that it remains
working as expected.
