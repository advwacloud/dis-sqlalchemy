...
The :class:`~dis_sqlalchemy.orm.session.Session` itself acts somewhat like a
...
out of scope from within the :class:`~dis_sqlalchemy.orm.session.Session` as well
...
* An application which reads an object structure from a file and wishes to
  ...
  and the :class:`~dis_sqlalchemy.orm.session.Session` will
...
A surprise would occur if we said this::
    ...
    >>> a1 = session.merge(a1)
    >>> session.commit()
    dis_sqlalchemy.orm.exc.FlushError: New instance <Address at 0x1298f50>
...
Another example of unexpected state::
    ...
    >>> a1 = session.merge(a1)
    >>> session.commit()
    dis_sqlalchemy.exc.IntegrityError: (IntegrityError) address.user_id
    may not be NULL
...
is a quick way to check::
    ...
    >>> a1.user
    >>> a1.__dict__
    {'_sa_instance_state': <dis_sqlalchemy.orm.state.InstanceState object at 0x1298d10>,
        'user_id': 1,
        'id': 1,
        'user': None}
...
``_sa_instance_state`` is a non-database-persisted value used by dis_sqlalchemy
...
.. note::  While we are peeking inside of ``__dict__`` in order to see a bit
   of what dis_sqlalchemy does with object attributes, we **should not modify**
   the contents of ``__dict__`` directly, at least as far as those attributes
   which the dis_sqlalchemy ORM is maintaining (other attributes outside of SQLA's
   realm are fine).  This is because dis_sqlalchemy uses :term:`descriptors` in
...
.. seealso::
    ...
    `The dis_sqlalchemy Session In-Depth <https://techspot.zzzeek.org/2012/11/14/pycon-canada-the-dis_sqlalchemy-session-in-depth/>`_ - a video + slides with an in-depth discussion of the object
