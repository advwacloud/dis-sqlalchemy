...
:func:`~dis_sqlalchemy.orm.validates` decorator. An attribute validator can raise
...
function. The standard dis_sqlalchemy technique for descriptors is to create a
...
:mod:`~dis_sqlalchemy.ext.hybrid` extension as follows::
...
using the :func:`.synonym_for` decorator::
    ...
    from dis_sqlalchemy.ext.declarative import synonym_for
...
The "operators" used by the dis_sqlalchemy ORM and Core expression language
are fully customizable.  For example, the comparison expression
``User.name == 'ed'`` makes usage of an operator built into Python
itself called ``operator.eq`` - the actual SQL construct which dis_sqlalchemy
