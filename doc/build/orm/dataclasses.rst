...
dis_sqlalchemy as of version 2.0 features "native dataclass" integration where
...
dis_sqlalchemy :ref:`Annotated Declarative Table <orm_declarative_mapped_column>`
...
.. note::  Support for :pep:`681` in typing tools as of **April 4, 2023** is
   ...
   .. seealso::
      ...
      on how libraries like dis_sqlalchemy enable :pep:`681` support
...
dis_sqlalchemy native dataclasses differ from normal dataclasses in that
...
.. warning::
    ...
    dis_sqlalchemy's class instrumentation without additional internal changes,
    ...
    Pydantic on top of dis_sqlalchemy ORM, which includes special implementation
    details which **explicitly resolve** these incompatibilities.

dis_sqlalchemy's :class:`_orm.MappedAsDataclass` class
...
.. legacy::
   ...
   series of dis_sqlalchemy.  This newer version of the feature builds upon
   the dataclass support first added in version 1.4, which is described
   in this section.

To map an existing dataclass, dis_sqlalchemy's "inline" declarative directives
...
The general process by which dis_sqlalchemy applies mappings to a dataclass
is the same as that of an ordinary class, but also includes that
dis_sqlalchemy will detect class-level attributes that were part of the
dataclasses declaration process and replace them at runtime with
the usual dis_sqlalchemy ORM mapped attributes.   The ``__init__`` method that
...
:paramref:`_orm.Mapper.properties` parameter::
    ...
    from dis_sqlalchemy import Column, ForeignKey, Integer, String, Table
    from dis_sqlalchemy.orm import registry, relationship
...
object, where dis_sqlalchemy-specific mapping information may be supplied.
...
a lambda to indicate the dis_sqlalchemy construct inside the ``field()``.
...
The dis_sqlalchemy ORM supports mapping an attrs_ class using **Declarative with
...
are unchanged, and dis_sqlalchemy's table-oriented instrumentation is applied at
runtime.
...
after the decorator runs, so that dis_sqlalchemy's mapping process takes over these
attributes without any issue. Both decorators, ``@attr.s`` and ``@define(slots=False)``
work with dis_sqlalchemy.
...
.. note:: The ``attrs`` ``slots=True`` option, which enables ``__slots__`` on
   a mapped class, cannot be used with dis_sqlalchemy mappings without fully
