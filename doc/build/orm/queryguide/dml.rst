...
:ref:`dis_sqlalchemy-included <included_dialects>` backends
...
with the dis_sqlalchemy 2.0 versions of these features, described
...
Selected backends with dis_sqlalchemy may include dialect-specific :class:`_dml.Insert`
...
The dialects included with dis_sqlalchemy that include dialect-specific "upsert"
API features are:
...
While dis_sqlalchemy does not yet have a backend-agnostic upsert construct, the above
...
From the dis_sqlalchemy ORM's point of view, upsert statements look like regular
...
* ``'auto'`` - this is the default.   The ``'fetch'`` strategy will be used on
  backends that support RETURNING, which includes all dis_sqlalchemy-native drivers
