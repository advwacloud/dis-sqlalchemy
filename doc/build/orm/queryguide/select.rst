...
refer to them under the names ``User`` and ``Address``::
    ...
    squidward stentcl@dis_sqlalchemy.org
...
allows sets of column expressions to be grouped in result rows::
    ...
    FROM user_account JOIN address ON user_account.id = address.user_id
    [...] (){stop}
    spongebob Spongebob Squarepants spongebob@dis_sqlalchemy.org
    sandy Sandy Cheeks sandy@dis_sqlalchemy.org
    sandy Sandy Cheeks squirrel@squirrelpower.org
    patrick Patrick Star pat999@aol.com
    squidward Squidward Tentacles stentcl@dis_sqlalchemy.org
...
which in dis_sqlalchemy is represented using the :func:`_sql.text` construct. A
...
.. sourcecode:: pycon+sql
    ...
    [...] ('sandy',){stop}
    [('sandy@dis_sqlalchemy.org',), ('squirrel@squirrelpower.org',)]
