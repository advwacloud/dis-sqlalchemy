...
A big part of dis_sqlalchemy is providing a wide range of control over how related
...
.. note::  The loader options applied to an object's lazy-loaded collections
   ...
   A future dis_sqlalchemy release may add more alternatives to manipulating
...
statements emitted.  In dis_sqlalchemy, the usual mitigation for the N+1 problem
...
the dis_sqlalchemy ORM.  It works by connecting a JOIN (by default
...
.. tip::
    ...
    This is not automatic in modern dis_sqlalchemy, as it changes the behavior
    of the result set to return fewer ORM objects than the statement would
    normally return in terms of number of rows.  Therefore dis_sqlalchemy keeps
...
to the subquery.   dis_sqlalchemy's joined eager loading goes the extra mile, and
...
* As "selectin" loading relies upon IN, for a mapping with composite primary
  ...
  version 3.15.  There is no special logic in dis_sqlalchemy to check
  ...
  advantage to dis_sqlalchemy just running the SQL out for it to fail is that if a
  particular database does start supporting this syntax, it will work without
  any changes to dis_sqlalchemy (as was the case with SQLite).
...
For this dis_sqlalchemy supplies the :func:`_orm.contains_eager`
...
.. tip::  dis_sqlalchemy now has a **much simpler way to do this**, by allowing
...
.. tip::  In all cases, the dis_sqlalchemy ORM does **not overwrite already loaded
...
.. autoclass:: dis_sqlalchemy.orm.Load
    :members:
