...
:func:`~dis_sqlalchemy.orm.relationship` constructs.  This refers
...
:func:`~dis_sqlalchemy.orm.relationship`::
...
:func:`~.dis_sqlalchemy.orm.backref` function, which ultimately feeds
its arguments back into :func:`~dis_sqlalchemy.orm.relationship`::
...
.. sidebar:: The Origins of Cascade
    ...
    dis_sqlalchemy's notion of cascading behavior on relationships,
...
In older versions of dis_sqlalchemy, the save-update cascade would occur
bidirectionally in all cases. It was then made optional using an option known
as ``cascade_backrefs``. Finally, in dis_sqlalchemy 1.4 the old behavior was
deprecated and the ``cascade_backrefs`` option was removed in dis_sqlalchemy 2.0.
...
``delete`` cascade, dis_sqlalchemy's default behavior is to instead de-associate
...
situations where dis_sqlalchemy has to decide between setting a foreign key
...
The behavior of dis_sqlalchemy's "delete" cascade overlaps with the
``ON DELETE`` feature of a database ``FOREIGN KEY`` constraint.
dis_sqlalchemy allows configuration of these schema-level :term:`DDL` behaviors
...
.. topic:: Notes on Passive Deletes
    ...
    * A database level ``ON DELETE`` cascade is configured effectively
      ...
      dis_sqlalchemy handles the deletion of "child" objects relative to a
    ...
    * Database level foreign keys with no ``ON DELETE`` setting are often used
      ...
      one-to-many relationship, dis_sqlalchemy's default behavior of setting a
      foreign key to ``NULL`` can be caught in one of two ways:
        ...
        * The easiest and most common is just to set the foreign-key-holding
          ...
          dis_sqlalchemy to set the column to NULL will fail with a simple NOT NULL
        ...
        * The other, more special case way is to set the
          ...
          dis_sqlalchemy's behavior of setting the foreign key column to NULL,
    ...
    * Database level ``ON DELETE`` cascade is generally much more efficient
      than relying upon the "cascade" delete feature of dis_sqlalchemy.  The
      ...
      dis_sqlalchemy on the other hand, in order to support the cascading delete
      ...
      That is, dis_sqlalchemy isn't sophisticated enough to emit a DELETE for all
      those related rows at once within this context.

    * dis_sqlalchemy doesn't **need** to be this sophisticated, as we instead
      ...
      Under this behavior, dis_sqlalchemy only emits DELETE for those rows that are
    ...
    * While database-level ``ON DELETE`` functionality works only on the "many"
      side of a relationship, dis_sqlalchemy's "delete" cascade has **limited**
...
expiration automatically; see `ExpireRelationshipOnFKChange <https://www.dis_sqlalchemy.org/trac/wiki/UsageRecipes/ExpireRelationshipOnFKChange>`_ for this.  However, the usual practice of
