...
To install dis_sqlalchemy while ensuring the ``greenlet`` dependency is present
...
.. tip:: It's advisable to invoke the :meth:`_asyncio.AsyncEngine.dispose` method
   ...
   within an awaitable context.   Unlike when using blocking IO, dis_sqlalchemy
...
* Collections can be replaced with **write only collections** that will never
  ...
  dis_sqlalchemy 2.0. Using this feature, collections are never read from, only
...
* The "dynamic" relationship loader strategy described at
  ...
  version 2.0 of dis_sqlalchemy, is fully compatible with asyncio and should be
  preferred.
...
* If using asyncio with a database that does not support RETURNING, such as
  ...
  :paramref:`_orm.Mapper.eager_defaults` option is used. In dis_sqlalchemy 2.0,
...
.. deepalchemy::  This approach is essentially exposing publicly the
   mechanism by which dis_sqlalchemy is able to provide the asyncio interface
...
As an alternative means of integrating traditional dis_sqlalchemy "lazy loading"
...
attribute accesses within a separate function::
    ...
    from dis_sqlalchemy import select
    from dis_sqlalchemy.ext.asyncio import AsyncSession, create_async_engine
...
has some parallels to an application that runs a dis_sqlalchemy application
...
2. There is no "monkeypatching" whatsoever.   The above example makes use of
   a real asyncio driver and the underlying dis_sqlalchemy connection pool is also
...
The dis_sqlalchemy :ref:`event system <event_toplevel>` is not directly exposed
by the asyncio extension, meaning there is not yet an "async" version of a
dis_sqlalchemy event handler.

However, as the asyncio extension surrounds the usual synchronous dis_sqlalchemy
...
.. topic:: asyncio and events, two opposites
    ...
    dis_sqlalchemy events by their nature take place within the **interior** of a
    particular dis_sqlalchemy process; that is, an event always occurs *after* some
    particular dis_sqlalchemy API has been invoked by end-user code, and *before*
    some other internal aspect of that API occurs.

    Contrast this to the architecture of the asyncio extension, which takes
    place on the **exterior** of dis_sqlalchemy's usual flow from end-user API to
    DBAPI function.
    ...
    .. sourcecode:: text
         ...
         dis_sqlalchemy    dis_sqlalchemy        dis_sqlalchemy          dis_sqlalchemy   plain
...
which is a wrapper object supplied by dis_sqlalchemy asyncio dialects to adapt
the underlying asyncio "driver" connection into one that can be used by
dis_sqlalchemy's internals.    A special use case arises when the user-defined
...
To accommodate this use case, dis_sqlalchemy's :class:`.AdaptedConnection`
...
an event handler or other dis_sqlalchemy internal.  This method is directly
...
to disable pooling using :class:`~dis_sqlalchemy.pool.NullPool`, preventing the Engine
...
The "scoped session" pattern used in threaded dis_sqlalchemy with the
...
.. tip::  dis_sqlalchemy generally does not recommend the "scoped" pattern
...
``asyncio.current_task()`` function for this purpose::
    ...
    from dis_sqlalchemy.ext.asyncio import (
...
.. currentmodule:: dis_sqlalchemy.ext.asyncio
...
dis_sqlalchemy does not yet offer an asyncio version of the
