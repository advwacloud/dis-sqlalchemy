...
A complete guide to dis_sqlalchemy ORM querying can be found at
...
When the :class:`~dis_sqlalchemy.orm.session.Session` is used with its default
...
synchronized with the current state of the transaction.   The dis_sqlalchemy
...
method explicitly, is as follows:
  ...
  * Objects which were initially in the :term:`pending` state when they were added
    to the :class:`~dis_sqlalchemy.orm.session.Session` within the lifespan of the
...
* Web applications.  In this case, it's best to make use of the dis_sqlalchemy
...
:class:`~dis_sqlalchemy.orm.session.Session` doesn't have to issue a query.
...
That's more the job of a **second level cache**.   dis_sqlalchemy provides
...
How can I get the :class:`~dis_sqlalchemy.orm.session.Session` for a certain object?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use the :meth:`~.Session.object_session` classmethod
available on :class:`~dis_sqlalchemy.orm.session.Session`::
...
When using the :class:`_asyncio.AsyncSession` object from dis_sqlalchemy's
...
The concurrency model for dis_sqlalchemy's :class:`_orm.Session` and
