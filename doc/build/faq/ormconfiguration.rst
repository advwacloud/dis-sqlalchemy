...
The dis_sqlalchemy ORM, in order to map to a particular table, needs there to be
...
goes far beyond that.  In dis_sqlalchemy, all ORM-mapped objects are at all times
linked uniquely within a :class:`.Session`
to their specific database row using a pattern called the :term:`identity map`,
a pattern that's central to the unit of work system employed by dis_sqlalchemy,
and is also key to the most common (and not-so-common) patterns of ORM usage.


.. note::

    It's important to note that we're only talking about the dis_sqlalchemy ORM; an
...
Given the example as follows::
    ...
    from dis_sqlalchemy import Integer, Column, ForeignKey
...
As of dis_sqlalchemy version 0.9.5, the above condition is detected, and will
...
dis_sqlalchemy implements :func:`_orm.subqueryload` by issuing a separate query,
