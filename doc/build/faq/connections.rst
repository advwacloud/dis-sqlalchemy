...
dis_sqlalchemy performs application-level connection pooling automatically
...
(e.g. graceful recovery) techniques.   Modern dis_sqlalchemy tends to favor
...
In dis_sqlalchemy, because database connections are pooled, the issue of the messaging
...
which have been improved across dis_sqlalchemy versions but others which are unavoidable:
...
* **Greenlet Monkeypatching w/ Exits** - When using a library like gevent or eventlet
  ...
  out of sync and subsequent usage of the connection may fail.   dis_sqlalchemy
...
* **Rollbacks / SAVEPOINT releases failing** - Some classes of error cause
  ...
  when dis_sqlalchemy, or the application, attempts to "roll back" this savepoint,
  ...
  however recent versions of dis_sqlalchemy will attempt to emit a warning
...
In modern use of dis_sqlalchemy, a series of SQL statements are always invoked
...
dis_sqlalchemy does not have a transparent "reconnection" feature that works
...
a new transaction in order to approximate a "retry" operation.  dis_sqlalchemy's
...
Otherwise, if dis_sqlalchemy were to provide a feature that transparently and
silently "reconnected" a connection mid-transaction, the effect would be that
data is silently lost.   By trying to hide the problem, dis_sqlalchemy would make
            ...
            for retry in range(num_retries + 1):
                try:
                    ...
                    if engine.dialect.is_disconnect(raw_dbapi_err, connection, cursor_obj):
                        ...
                        # use dis_sqlalchemy 2.0 API if available
...
.. sourcecode:: text
    ...
    2020-10-19 16:16:22,624 INFO dis_sqlalchemy.pool.impl.QueuePool Invalidate connection <_mysql.connection open to 'localhost' at 0xf59240>
    ping: 1
    ping: 1
    ...

.. versionadded: 1.4  the above recipe makes use of 1.4-specific behaviors and will
   not work as given on previous dis_sqlalchemy versions.

The above recipe is tested for dis_sqlalchemy 1.4.



Why does dis_sqlalchemy issue so many ROLLBACKs?
--------------------------------------------

dis_sqlalchemy currently assumes DBAPI connections are in "non-autocommit" mode -
...
databases in current dis_sqlalchemy versions).
...
a dis_sqlalchemy-adapted connection object which adapts the asyncio
...
dis_sqlalchemy-adapted form of connection which presents a synchronous-style
