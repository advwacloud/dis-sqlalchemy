...
dis_sqlalchemy ORM, see :ref:`session_transaction_isolation`.
...
does not properly handle the exception.    For example::
    ...
    from dis_sqlalchemy import create_engine, Column, Integer
...
What dis_sqlalchemy offers that solves both issues is support of SAVEPOINT, via
...
See the recipe at `FilteredQuery <https://www.dis_sqlalchemy.org/trac/wiki/UsageRecipes/FilteredQuery>`_.
...
dis_sqlalchemy may alter this behavior on :class:`_query.Query`, which may also involve
...
I'm using ``joinedload()`` or ``lazy=False`` to create a JOIN/OUTER JOIN and dis_sqlalchemy is not constructing the correct query when I try to add a WHERE, ORDER BY, LIMIT, etc. (which relies upon the (OUTER) JOIN)
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
dis_sqlalchemy will always issue UPDATE or DELETE statements for dependent
...
To configure dis_sqlalchemy to cooperate with ON DELETE CASCADE, see
:ref:`passive_deletes`.
...
The recipe `ExpireRelationshipOnFKChange <https://www.dis_sqlalchemy.org/trac/wiki/UsageRecipes/ExpireRelationshipOnFKChange>`_ features an example using dis_sqlalchemy events
...
This `UniqueObject <https://www.dis_sqlalchemy.org/trac/wiki/UsageRecipes/UniqueObject>`_ recipe was created to address this issue.
