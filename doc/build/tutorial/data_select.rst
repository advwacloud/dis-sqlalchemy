...
it with full ``Address`` entities in the second element::
    ...
    [...] (){stop}
    [('spongebob', Address(id=1, email_address='spongebob@dis_sqlalchemy.org')),
    ('sandy', Address(id=2, email_address='sandy@dis_sqlalchemy.org')),
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import func, cast
    >>> stmt = select(
...
dis_sqlalchemy allows us to compose SQL expressions, such as ``name = 'squidward'``
...
of ORM entities::
    ...
    >>> from dis_sqlalchemy import and_, or_
    >>> print(
...
.. seealso::
    ...
    :doc:`/core/operators` - descriptions of most SQL operator functions in dis_sqlalchemy
...
``count(*)``, we use a dis_sqlalchemy element known as :attr:`_sql.func` to
...
.. tip::
    ...
    SQL also has a "RIGHT OUTER JOIN".  dis_sqlalchemy doesn't render this directly;
...
dis_sqlalchemy provides for SQL functions in an open-ended way using a namespace
...
dis_sqlalchemy provides for these two clauses using the :meth:`_sql.Select.group_by`
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import func, desc
    >>> stmt = (
...
In the dis_sqlalchemy Expression Language, these "names" are instead represented by
...
dis_sqlalchemy uses the :class:`_sql.Subquery` object to represent a subquery and
...
Even though a SQL subquery does not itself have any constraints, dis_sqlalchemy can
...
Usage of the :class:`_sql.CTE` construct in dis_sqlalchemy is virtually
...
.. sourcecode:: pycon+sql
    ...
    [...] ('%@aol.com',){stop}
    User(id=1, name='spongebob', fullname='Spongebob Squarepants') Address(id=1, email_address='spongebob@dis_sqlalchemy.org')
    User(id=2, name='sandy', fullname='Sandy Cheeks') Address(id=2, email_address='sandy@dis_sqlalchemy.org')
...
dis_sqlalchemy represents the scalar subquery using the
...
However, in the case where the correlation is ambiguous, dis_sqlalchemy will let
...
.. sourcecode:: pycon+sql
    ...
    [...] (){stop}
    [('spongebob', 'spongebob@dis_sqlalchemy.org', 1), ('sandy', 'sandy@dis_sqlalchemy.org', 2),
...
dis_sqlalchemy supports this feature using the :meth:`_expression.Select.lateral`
...
dis_sqlalchemy's :class:`_sql.Select` construct supports compositions of this
...
collection that may be referred towards in an enclosing :func:`_sql.select`::
    ...
    [generated in ...] ('sandy', 'spongebob')
    {stop}[('sandy', 'sandy@dis_sqlalchemy.org'), ('sandy', 'sandy@squirrelpower.org'), ('spongebob', 'spongebob@dis_sqlalchemy.org')]
    {execsql}ROLLBACK{stop}
...
the SELECT statement would return a row.  dis_sqlalchemy includes a variant of the
...
* the ``now()`` function, which provides for the current date and time; as this
  is a common function, dis_sqlalchemy knows how to render this differently for each
...
statement and getting rows back, for those cases where dis_sqlalchemy has to apply
result-set processing. A prime example of this are date-related functions on
SQLite, where dis_sqlalchemy's :class:`_types.DateTime` and related datatypes take
...
However, for the vast majority of SQL functions, dis_sqlalchemy does not have them
...
and ``func.upper()`` to convert the casing of strings, dis_sqlalchemy doesn't
...
special type handling on the dis_sqlalchemy side, and dis_sqlalchemy's type
...
1. the function is not already a dis_sqlalchemy built-in function; this can be
...
In dis_sqlalchemy, all SQL functions generated by the :data:`_sql.func` namespace
...
.. sourcecode:: pycon+sql
    ...
    FROM user_account JOIN address ON user_account.id = address.user_id
    [...] ()
    {stop}[(1, 'sandy', 'sandy@dis_sqlalchemy.org'), (2, 'sandy', 'sandy@squirrelpower.org'), (1, 'spongebob', 'spongebob@dis_sqlalchemy.org')]
...
.. sourcecode:: pycon+sql
    ...
    FROM user_account JOIN address ON user_account.id = address.user_id
    [...] ()
    {stop}[(2, 'sandy', 'sandy@dis_sqlalchemy.org'), (2, 'sandy', 'sandy@squirrelpower.org'), (3, 'spongebob', 'spongebob@dis_sqlalchemy.org')]
    {printsql}ROLLBACK{stop}
...
and ``rank()``.  dis_sqlalchemy includes built in implementations
...
dis_sqlalchemy provides the :meth:`_functions.FunctionElement.table_valued` method
...
dis_sqlalchemy refers to this as a "column valued" function and is available
...
this task, which in dis_sqlalchemy is provided by the :func:`.cast` function.
...
produces a dis_sqlalchemy column expression that will act as the given datatype on
...
Sometimes there is the need to have dis_sqlalchemy know the datatype of an
