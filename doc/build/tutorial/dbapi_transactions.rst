...
.. container:: orm-header
    ...
    **Note to ORM readers**
    ...
    modern dis_sqlalchemy emphasizes a transactional and SQL execution pattern that
...
As we have yet to introduce the dis_sqlalchemy Expression Language that is the
primary feature of dis_sqlalchemy, we will make use of one simple construct within
...
day-to-day dis_sqlalchemy use is by far the exception rather than the rule for most
...
.. sourcecode:: pycon+sql
    ...
    [...] ()
    <dis_sqlalchemy.engine.cursor.CursorResult object at 0x...>
...
again for subsequent statements.  dis_sqlalchemy refers to this style as **commit as
you go**.
...
.. topic::  What's "BEGIN (implicit)"?
    ...
    transaction block.  "implicit" here means that dis_sqlalchemy **did not
...
.. [1] :term:`DDL` refers to the subset of SQL that instructs the database
   ...
   as we'll see later, we usually let dis_sqlalchemy run DDL sequences for us as
...
dis_sqlalchemy abstracts these formats into just one, which is the "named" format
using a colon.

.. topic:: Always use bound parameters

    As mentioned at the beginning of this section, textual SQL is not the usual
    way we work with dis_sqlalchemy.  However, when using textual SQL, a Python
    ...
    when the data is untrusted.  However it also allows the dis_sqlalchemy dialects
    and/or DBAPI to correctly handle the incoming input for the backend.
    Outside of plain textual SQL use cases, dis_sqlalchemy's Core Expression API
...
:meth:`_sql.Insert.returning` method.  In that case, dis_sqlalchemy makes use of
...
ORM is called the :class:`_orm.Session`.  In modern dis_sqlalchemy, this object
