...
The ``None`` value is provided by dis_sqlalchemy to indicate that the attribute
has no value as of yet.  dis_sqlalchemy-mapped attributes always return a value
...
directly, and their usage resembles how they are used with dis_sqlalchemy Core
...
with the exception of a special dis_sqlalchemy internal state object::

    >>> sandy.__dict__
    {'_sa_instance_state': <dis_sqlalchemy.orm.state.InstanceState object at 0x...>}
...
* It **expunges** all objects from the :class:`_orm.Session`.
  ...
  refreshed::
    ...
    Traceback (most recent call last):
      ...
    dis_sqlalchemy.orm.exc.DetachedInstanceError: Instance <User at 0x...> is not bound to a Session; attribute refresh operation cannot proceed
