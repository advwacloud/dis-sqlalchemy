...
The central element of both dis_sqlalchemy Core and ORM is the SQL Expression
...
The most common foundational objects for database metadata in dis_sqlalchemy are
...
In dis_sqlalchemy, the database "table" is ultimately represented
by a Python object similarly named :class:`_schema.Table`.

To start using the dis_sqlalchemy Expression Language, we will want to have
...
dis_sqlalchemy tutorial model, which has a table called ``user_account`` that
...
each other, dis_sqlalchemy uses the presence of these foreign key constraint
...
.. topic:: Migration tools are usually appropriate
    ...
    <https://alembic.dis_sqlalchemy.org>`_, which builds upon dis_sqlalchemy, is likely
...
.. topic:: Another way to make Table objects?
  ...
  object, which underlies how dis_sqlalchemy ultimately refers to database tables
  when constructing SQL expressions. As mentioned, the dis_sqlalchemy ORM provides
  ...
  dis_sqlalchemy can also be used quite effectively with Core-centric
  use as well.
...
dis_sqlalchemy :class:`_orm.DeclarativeBase` class::
...
is the central "mapper configuration" unit in the dis_sqlalchemy ORM.  While
...
via the :attr:`_orm.DeclarativeBase.registry` class variable::
    ...
    >>> Base.registry
    <dis_sqlalchemy.orm.decl_api.registry object at 0x...>
...
.. topic::  Where'd the old Declarative go?
    ...
    Users of dis_sqlalchemy 1.4 or previous will note that the above mapping
    ...
    deriving the declarations from type annotations is part of dis_sqlalchemy's
...
.. tip::  There is no requirement that reflection must be used in order to
   use dis_sqlalchemy with a pre-existing database.  It is entirely typical that
   the dis_sqlalchemy application declares all metadata explicitly in Python,
