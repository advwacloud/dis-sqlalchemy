==============================
What's New in dis_sqlalchemy 0.9?
==============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 0.8,
    undergoing maintenance releases as of May, 2013,
    and dis_sqlalchemy version 0.9, which had its first production
...
This guide introduces what's new in dis_sqlalchemy version 0.9,
and also documents changes which affect users migrating
their applications from the 0.8 series of dis_sqlalchemy to 0.9.
...
Python 3.   All dis_sqlalchemy modules and unit tests are now interpreted
...
.. sourcecode:: sql
    ...
    -- dis_sqlalchemy 0.8 and earlier...
...
This is a lesser used behavior that in modern dis_sqlalchemy is roughly equivalent
...
So with dis_sqlalchemy 0.9, our query that selects from ``select_stmt`` produces
the SQL we expect:
...
The :meth:`_query.Query.select_entity_from` method will be available in dis_sqlalchemy
...
The change is illustrated as follows::
    ...
    from dis_sqlalchemy import Column, Integer, ForeignKey, create_engine
    from dis_sqlalchemy.orm import backref, relationship, Session
...
fact that missing many-to-ones return None in dis_sqlalchemy, so should the
proxied value.  E.g.::
...
This is a small change demonstrated as follows::
    ...
    from dis_sqlalchemy import Column, Integer, String, create_engine, inspect
    from dis_sqlalchemy.orm import Session, attributes
    from dis_sqlalchemy.ext.declarative import declarative_base
...
arguments which were silently ignored::
    ...
    from dis_sqlalchemy import Date, Integer
...
.. sourcecode:: text
    ...
    $ python -W always::DeprecationWarning ~/dev/dis_sqlalchemy/test.py
    /Users/classic/dev/dis_sqlalchemy/test.py:5: SADeprecationWarning: Passing arguments to
    type object constructor <class 'dis_sqlalchemy.types.Date'> is deprecated
      d = Date(storage_format="%(day)02d.%(month)02d.%(year)04d")
    /Users/classic/dev/dis_sqlalchemy/test.py:9: SADeprecationWarning: Passing arguments to
    type object constructor <class 'dis_sqlalchemy.types.Integer'> is deprecated
      i = Integer(display_width=5)
...
dialect-specific types::
    ...
    from dis_sqlalchemy.dialects.sqlite import DATE
    from dis_sqlalchemy.dialects.mysql import INTEGER
...
:meth:`.TypeEngine.with_variant` isn't new, it was added in dis_sqlalchemy
...
This pattern was not a documented pattern even though some dis_sqlalchemy internals
made use of it::
...
backends that support boolean constants::
    ...
    from dis_sqlalchemy.sql import true
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy.sql import table, column
...
The conversion which dis_sqlalchemy does whenever a DBAPI returns a Python
...
10 takes place.  E.g.::
    ...
    from dis_sqlalchemy.dialects.mysql import DOUBLE
    import decimal
...
from a backref::
    ...
    from dis_sqlalchemy.orm import relationship, validates
    from dis_sqlalchemy.ext.declarative import declarative_base
...
A new extension is added in **0.9.1** known as :mod:`dis_sqlalchemy.ext.automap`.  This is an
...
illustrates how :mod:`dis_sqlalchemy.ext.automap` is able to deliver mapped
classes, including relationships, based on a reflected schema::

    from dis_sqlalchemy.ext.automap import automap_base
...
For many years, the dis_sqlalchemy ORM has been held back from being able to nest
...
but dis_sqlalchemy has always had a simple rewriting scheme in place for Oracle's syntax).
To make matters worse, dis_sqlalchemy's usual workaround of applying a
...
when :func:`_orm.joinedload` is used similarly, dis_sqlalchemy's ORM would always make sure a nested
...
As of dis_sqlalchemy 0.9, an entirely new approach has been taken.  The ORM no longer
...
.. note::
    ...
    As of dis_sqlalchemy 1.1, the workarounds present in this feature for SQLite
...
Starting with a table such as this::
    ...
    from dis_sqlalchemy import Table, Boolean, Integer, Column, MetaData
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import select, and_, false, true
    >>> from dis_sqlalchemy.dialects import mysql, postgresql
...
1. The type on a :class:`_schema.Column` is immediately present as soon as the
   ...
   this works no matter which side is configured first::
    ...
    >>> from dis_sqlalchemy import Table, MetaData, Column, Integer, ForeignKey
    >>> metadata = MetaData()
...
2. The system now works with :class:`_schema.ForeignKeyConstraint` as well::
    ...
    >>> from dis_sqlalchemy import Table, MetaData, Column, Integer, ForeignKeyConstraint
    >>> metadata = MetaData()
    >>> t2 = Table(
    ...     "t2",
    ...     metadata,
...
.. seealso::
    ...
    :mod:`dis_sqlalchemy.dialects.firebird.fdb`

    :mod:`dis_sqlalchemy.dialects.firebird.kinterbasdb`
