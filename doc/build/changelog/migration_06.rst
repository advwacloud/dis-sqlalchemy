=============================
What's New in dis_sqlalchemy 0.6?
=============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 0.5,
    last released January 16, 2010, and dis_sqlalchemy version 0.6,
...
dis_sqlalchemy to 0.6.  Note that dis_sqlalchemy 0.6 removes some
...
* cPython 3.x - see [source:dis_sqlalchemy/trunk/README.py3k]
...
``dis_sqlalchemy.dialects`` package.  The
``dis_sqlalchemy.databases`` package still exists as a
placeholder to provide some level of backwards compatibility
for simple imports.

For each supported database, a sub-package exists within
``dis_sqlalchemy.dialects`` where several files are contained.
...
dis_sqlalchemy dialects are first declared in the ``base.py``
...
``dis_sqlalchemy.connectors`` package, which provide
...
* Documentation on connect arguments:
  https://www.dis_sqlalchemy.org/docs/06/dbengine.html#create-
  engine-url-arguments.

* Reference documentation for individual dialects: https://ww
  w.dis_sqlalchemy.org/docs/06/reference/dialects/index.html
...
* the type system has been changed dramatically in
  dis_sqlalchemy 0.6.  This has an impact on all dialects
...
* the setuptools entrypoint used to locate external dialects
  is now called ``dis_sqlalchemy.dialects``.  An external
...
``dis_sqlalchemy.dialects.<name>``.  For example, to import a
set of PG types:
...
from ``dis_sqlalchemy.types``, but the PG dialect makes it
...
::
    ...
    >>> column("foo") == 5
    <dis_sqlalchemy.sql.expression._BinaryExpression object at 0x1252490>
...
In previous versions of dis_sqlalchemy, the returned
...
* Support for correct hashing of ``ClauseElement`` objects
  ...
  this point dis_sqlalchemy relied heavily on the specific
...
An "executemany" in dis_sqlalchemy corresponds to a call to
...
column.  Previous versions of dis_sqlalchemy would simply insert
...
dis_sqlalchemy 0.6 therefore establishes predictable consistency
...
the end of
[https://www.dis_sqlalchemy.org/docs/06/sqlexpression.html
...
dis_sqlalchemy's path to the "dark side" where we hope to
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.sql select: 0.360s
    dis_sqlalchemy.orm fetch: 2.500s

    0.6 / Pure Python

    dis_sqlalchemy.sql select: 0.600s
    dis_sqlalchemy.orm fetch: 3.000s

    0.5 / Pure Python

    dis_sqlalchemy.sql select: 0.790s
    dis_sqlalchemy.orm fetch: 4.030s
...
The ``dis_sqlalchemy.schema`` package has received some long-
needed attention.   The most visible change is the newly
expanded DDL system.   In dis_sqlalchemy, it was possible since
...
and using the ``dis_sqlalchemy.ext.compiler`` extension you can
make your own:
...
``dis_sqlalchemy.schema`` constructs is centralized and the
...
* Some backends, such as Oracle, only support RETURNING that
  ...
  Oracle, not dis_sqlalchemy).

RETURNING is also used automatically by dis_sqlalchemy, when
...
dis_sqlalchemy allows the DBAPI and backend database in use to
...
cases where dis_sqlalchemy does need to coerce a bind parameter
...
over the contents of the cache.   dis_sqlalchemy 0.6 includes
...
* the 'echo_uow' flag on Session is removed.  Use   logging
  on the "dis_sqlalchemy.orm.unitofwork" name.
...
* ``ScopedSession.mapper`` remains deprecated.  See the
  usage recipe at   https://www.dis_sqlalchemy.org/trac/wiki/Usag
  eRecipes/SessionAwareMapper

* passing an ``InstanceState`` (internal dis_sqlalchemy state
...
`MixIn class <https://www.dis_sqlalchemy.org/docs/reference/ext/d
