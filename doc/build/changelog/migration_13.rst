=============================
What's New in dis_sqlalchemy 1.3?
=============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 1.2
    and dis_sqlalchemy version 1.3.

Introduction
============

This guide introduces what's new in dis_sqlalchemy version 1.3
and also documents changes which affect users migrating
their applications from the 1.2 series of dis_sqlalchemy to 1.3.
...
As dis_sqlalchemy has 14 years of real world use by thousands of developers, it's
...
The larger context is that dis_sqlalchemy seeks to adjust to the coming Python
3-only world, as well as a type-annotated world, and towards this goal there
are **tentative** plans for a major rework of  dis_sqlalchemy which would hopefully
...
As these two systems evolved dramatically after dis_sqlalchemy's first release, in
...
mapper has its roots in the 0.1, 0.2 series of dis_sqlalchemy where it was
...
good thing since dis_sqlalchemy also moved away from "classical" mappings altogether
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.InvalidRequestError: Can't determine which FROM clause to
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.InvalidRequestError: Don't know how to join from
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.IdentifierError: Identifier
...
To apply dis_sqlalchemy-side truncation rules to the above identifier, use the
:func:`.conv` construct::
...
"SELECT 1 FROM (SELECT 1) as _empty_set WHERE 1!=1" for MySQL::
    ...
    >>> from dis_sqlalchemy import select, literal_column, bindparam
...
such as on PostgreSQL::
    ...
    >>> from dis_sqlalchemy import create_engine
    >>> from dis_sqlalchemy import select, literal_column, tuple_, bindparam
    >>> e = create_engine("postgresql://scott:tiger@localhost/test", echo=True)
    >>> with e.connect() as conn:
    ...     conn.execute(
    ...         select([literal_column("1")]).where(
...
variant expression in order to locate these methods::
    ...
    from dis_sqlalchemy import TypeDecorator, LargeBinary, func
...
The "threadlocal engine strategy" was added around dis_sqlalchemy 0.2, as a
solution to the problem that the standard way of operating in dis_sqlalchemy 0.1,
which can be summed up as "threadlocal everything",  was found to be lacking.
In retrospect, it seems fairly absurd that by dis_sqlalchemy's first releases which
...
The original usage model for dis_sqlalchemy looked like this::
...
be removed totally by the next major series of dis_sqlalchemy, e.g. 1.4.  The
...
these parameters was to instruct dis_sqlalchemy to ensure that incoming Python
...
they did provide.    Eventually, dis_sqlalchemy added C extensions, one of the
...
longer be needed in any circumstances, and if they are, the dis_sqlalchemy project
...
dis_sqlalchemy can render the "PARTITION BY" sequence within a PostgreSQL
...
previously with this behavior under Python 2, dis_sqlalchemy's very performant
...
* The value of the ``threaded`` parameter, which has always been defaulted
  to True for the dis_sqlalchemy dialect, is no longer generated by default.
  The dis_sqlalchemy :class:`_engine.Connection` object is not considered to be thread-safe
...
``CREATE SEQUENCE`` syntax.  In :ticket:`4235`, dis_sqlalchemy will add support for
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.StatementError: (dis_sqlalchemy.exc.InvalidRequestError) A value is
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.StatementError: (dis_sqlalchemy.exc.InvalidRequestError) A value is required for bind parameter 'id'
...
"error" portion that is generated from the DBAPI driver or dis_sqlalchemy internals
