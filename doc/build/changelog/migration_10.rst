=============================
What's New in dis_sqlalchemy 1.0?
=============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 0.9,
    undergoing maintenance releases as of May, 2014,
    and dis_sqlalchemy version 1.0, released in April, 2015.
...
This guide introduces what's new in dis_sqlalchemy version 1.0,
and also documents changes which affect users migrating
their applications from the 0.9 series of dis_sqlalchemy to 1.0.
...
on the abstract base::
    ...
    from dis_sqlalchemy.ext.declarative import (
        declarative_base,
        declared_attr,
...
of load that's improved the most::
    ...
    import time
    from dis_sqlalchemy import Integer, Column, create_engine, Table
...
taken up by dis_sqlalchemy's objects, associated dictionaries, as
well as weakrefs, within a basic import of "nova.db.dis_sqlalchemy.models":

.. sourcecode:: text

    # reported by heapy, summation of dis_sqlalchemy objects +
...
"auto-attach" itself to a :class:`_schema.Table` based on being passed table-attached columns::
    ...
    from dis_sqlalchemy import Table, Column, MetaData, Integer, UniqueConstraint
...
expressions are rendered as constants into the SELECT statement::
    ...
    from dis_sqlalchemy import Table, Column, MetaData, Integer, select, func
...
embedded in SQL to render correctly, such as::
    ...
    from dis_sqlalchemy import Table, Column, MetaData, Text
    from dis_sqlalchemy.schema import CreateTable
    from dis_sqlalchemy.dialects.postgresql import ARRAY, array
...
emitted for ten of the parameter sets, out of a total of 1000::
    ...
    from dis_sqlalchemy import create_engine, Unicode, select, cast
    import random
    import warnings
...
.. sourcecode:: text
    ...
    /path/lib/dis_sqlalchemy/sql/sqltypes.py:186: SAWarning: Unicode type received
...
This pattern is not currently supported in dis_sqlalchemy.  For all versions,
...
someday, dis_sqlalchemy might fix this issue to generate "IS NULL", and the queries
...
It has always been dis_sqlalchemy's behavior such that if we access a scalar
...
the same as NULL in most cases.   Whether dis_sqlalchemy received a history
...
Starting with a mapping as::
    ...
    from dis_sqlalchemy import Integer, Column, String, ForeignKey
    from dis_sqlalchemy.orm import Session, relationship
    from dis_sqlalchemy.ext.declarative import declarative_base
...
originated in the very early 0.1 / 0.2 series of dis_sqlalchemy and have long
...
with right-nested joins is SQLite; dis_sqlalchemy as of 0.9 converts a right-nested
...
dis_sqlalchemy considers the relationship ``A.b`` to be a "one to many,
...
A warning is emitted in dis_sqlalchemy 0.9.10 (not yet released as of
...
Since dis_sqlalchemy's inception, there has always been an emphasis on not getting
...
to get rid of the warnings we would rewrite our statement as follows::
    ...
    from dis_sqlalchemy import select, text
...
about the text if we use :func:`_expression.column` and :func:`.table`::
    ...
    from dis_sqlalchemy import select, text, column, table
...
be imported from "dis_sqlalchemy" without the "sql" part.
...
The upshot of the whole change is that dis_sqlalchemy now would like us
...
label name in an order by, group by, or other expression, dis_sqlalchemy expects
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.CompileError: INSERT value for column my_table.data is
...
dis_sqlalchemy's wrapping of DBAPI exceptions was not taking place in the
...
dis_sqlalchemy's Core model is based on object identity in order to
...
method that returns information on all available ``ENUM`` types::
    ...
    from dis_sqlalchemy import inspect, create_engine
...
by PostgreSQL as of 9.4.  dis_sqlalchemy allows this using
...
.. seealso::
    ...
    :mod:`dis_sqlalchemy.dialects.postgresql.psycopg2cffi`
...
dis_sqlalchemy now emits NULL/NOT NULL unconditionally for all TIMESTAMP
columns.
...
dis_sqlalchemy specific exceptions.
...
dialect, available at https://bitbucket.org/zzzeek/dis_sqlalchemy-drizzle.
This dialect was added to dis_sqlalchemy right before dis_sqlalchemy was able to
...
MySQL + MySQLdb dialects within dis_sqlalchemy.  The dialect is as of yet
...
with an explicit hostname, now requires a driver name - dis_sqlalchemy will no
...
dis_sqlalchemy's previously hardcoded default of "SQL Server" is obsolete on
Windows, and dis_sqlalchemy cannot be tasked with guessing the best driver
