=============================
What's New in dis_sqlalchemy 0.8?
=============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 0.7,
    undergoing maintenance releases as of October, 2012,
    and dis_sqlalchemy version 0.8, which is expected for release
    in early 2013.
...
This guide introduces what's new in dis_sqlalchemy version 0.8,
and also documents changes which affect users migrating
their applications from the 0.7 series of dis_sqlalchemy to 0.8.

dis_sqlalchemy releases are closing in on 1.0, and each new
...
dis_sqlalchemy 0.8 will target Python 2.5 and forward;
...
dis_sqlalchemy will eventually drop 2.5 support as well - when
2.6 is reached as the baseline, dis_sqlalchemy will move to use
...
* relationships against self-referential, composite foreign
  ...
  to ``folder_id``.  When dis_sqlalchemy constructs an auto-
...
* Previously difficult custom join conditions, like those involving
  ...
  dis_sqlalchemy concept known as :term:`annotations`.  These annotations
...
Lots of dis_sqlalchemy users are writing systems that require
...
system.  While dis_sqlalchemy ORM models are also fully
...
is one already part of dis_sqlalchemy's system,
...
.. sourcecode:: pycon+sql
    ...
    [<dis_sqlalchemy.orm.properties.RelationshipProperty object at 0x1015212d0>]

    >>> # they are also namespaces
    >>> b.column_attrs.id
    <dis_sqlalchemy.orm.properties.ColumnProperty object at 0x101525090>
    ...
    >>> b
    <dis_sqlalchemy.orm.attributes.InstrumentedAttribute object at 0x101521fd0>
    ...
    >>> b
    <dis_sqlalchemy.orm.state.InstanceState object at 0x10152bed0>
    ...
    >>> b.attrs.id
    <dis_sqlalchemy.orm.state.AttributeState object at 0x10152bf90>
    ...
    >>> # owning session
    >>> b.session
    <dis_sqlalchemy.orm.session.Session object at 0x101701150>
...
Note that the dis_sqlalchemy APIs used by the Dogpile example as well
...
::
    ...
    from dis_sqlalchemy.types import Numeric
...
functionality, except on the database side::
    ...
    from dis_sqlalchemy import func, Table, Column, MetaData
...
The new behavior allows the following test case to work::
    ...
    from dis_sqlalchemy import Column, Integer, String, ForeignKey
    from dis_sqlalchemy.orm import relationship, backref
    from dis_sqlalchemy.ext.declarative import declarative_base
...
objects relative to what's being selected::
    ...
    from dis_sqlalchemy.sql import table, column, select
...
overall this level of guessing is outside of dis_sqlalchemy's
...
dis_sqlalchemy functions, so these calls will be changed to no
...
The ``dis_sqlalchemy.orm.interfaces.InstrumentationManager``
class is moved to
``dis_sqlalchemy.ext.instrumentation.InstrumentationManager``.
...
point it is injected back into ``dis_sqlalchemy.orm`` by
...
interface on top of the dis_sqlalchemy ORM.   SQLSoup is now
...
The older "mutable" system within the dis_sqlalchemy ORM has
...
dis_sqlalchemy versions has provided a way for the ORM to detect
...
In 0.7, the `dis_sqlalchemy.ext.mutable <https://docs.dis_sqlalchemy.
...
dis_sqlalchemy.exceptions (has been dis_sqlalchemy.exc for years)
---------------------------------------------------------

We had left in an alias ``dis_sqlalchemy.exceptions`` to attempt
to make it slightly easier for some very old libraries that
hadn't yet been upgraded to use ``dis_sqlalchemy.exc``.  Some
