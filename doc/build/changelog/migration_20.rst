...
======================================
dis_sqlalchemy 2.0 - Major Migration Guide
======================================

.. admonition:: Note for Readers

    dis_sqlalchemy 2.0's transition documents are separated into **two**
    ...
    to dis_sqlalchemy 1.4:

    * :ref:`migration_20_toplevel` - this document, 1.x to 2.x API shifts
    * :ref:`whatsnew_20_toplevel` - new features and behaviors for dis_sqlalchemy 2.0

    Readers who have already updated their 1.4 application to follow
    dis_sqlalchemy 2.0 engine and ORM conventions may navigate to
    :ref:`whatsnew_20_toplevel` for an overview of new features and
    capabilities.

.. admonition:: About this document

    This document describes changes between dis_sqlalchemy version 1.4
    and dis_sqlalchemy version 2.0.

    dis_sqlalchemy 2.0 presents a major shift for a wide variety of key
    dis_sqlalchemy usage patterns in both the Core and ORM components.   The goal
    of this release is to make a slight readjustment in some of the most
    fundamental assumptions of dis_sqlalchemy since its early beginnings, and to
    ...
    dis_sqlalchemy started with Python 2.3 which had no context managers, no
    ...
    dis_sqlalchemy 2.0 are targeting the residual assumptions left over from this
    early period in dis_sqlalchemy's development as well as the leftover artifacts
...
to be "dis_sqlalchemy 2.0" were in fact released as fully available within the 1.4
...
in dis_sqlalchemy 1.4. in the :ref:`migration_14_toplevel` document.

For dis_sqlalchemy 2.0, all API features and behaviors
...
a transition path first into the 1.4 series of dis_sqlalchemy where the "future"
...
The dis_sqlalchemy 2.0 transition presents itself in the dis_sqlalchemy 1.4 release as
a series of steps that allow an application of any size or complexity to be
migrated to dis_sqlalchemy 2.0 using a gradual, iterative process.  Lessons learned
...
**mostly** cross-compatible with dis_sqlalchemy 2.0. **Please note there may be
additional API and behavioral changes that may behave differently when running
against dis_sqlalchemy 2.0; always test code against an actual dis_sqlalchemy 2.0
...
a typical non trivial application, is to ensure it runs on dis_sqlalchemy 1.3 with
...
tests, with the latest dis_sqlalchemy 1.3 release with no dis_sqlalchemy deprecation
...
Once the application is good to go on dis_sqlalchemy 1.3, the next step is to get
it running on dis_sqlalchemy 1.4.  In the vast majority of cases, applications
should run without problems from dis_sqlalchemy 1.3 to 1.4.   However, it's always
the case between any 1.x and 1.y release, APIs and behaviors have changed
either subtly or in some cases a little less subtly, and the dis_sqlalchemy
...
For the full overview of dis_sqlalchemy 1.4 changes, see the
:doc:`/changelog/migration_14` document.

Migration to 2.0 Step One - Python 3 only (Python 3.7 minimum for 2.0 compatibility)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

dis_sqlalchemy 2.0 was first inspired by the fact that Python 2's EOL was in 2020.
dis_sqlalchemy is taking a longer period of time than other major projects to drop
Python 2.7 support. However, in order to use dis_sqlalchemy 2.0, the application
will need to be runnable on at least **Python 3.7**. dis_sqlalchemy 1.4 supports
...
dis_sqlalchemy 1.4 features a conditional deprecation warning system inspired
by the Python "-3" flag that would indicate legacy patterns in a running
application.   For dis_sqlalchemy 1.4, the :class:`_exc.RemovedIn20Warning`
deprecation class is emitted only when an environment variable
``dis_sqlalchemy_WARN_20`` is set to either of ``true`` or ``1``.
...
To enable "2.0 deprecations mode", we enable the ``dis_sqlalchemy_WARN_20=1``
variable, and additionally ensure that a `warnings filter`_ that will not
suppress any warnings is selected:

.. sourcecode:: text

    dis_sqlalchemy_WARN_20=1 python -W always::DeprecationWarning test3.py
...
.. sourcecode:: text
  ...
  $ dis_sqlalchemy_WARN_20=1 python2 -W always::DeprecationWarning test3.py
  test3.py:9: RemovedIn20Warning: The Engine.execute() function/method is considered legacy as of the 1.x series of dis_sqlalchemy and will be removed in 2.0. All statement execution in dis_sqlalchemy 2.0 is performed by the Connection.execute() method of Connection, or in the ORM by the Session.execute() method of Session. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9) (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    engine.execute("CREATE TABLE foo (id integer)")
  /home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/engine/base.py:2856: RemovedIn20Warning: Passing a string to Connection.execute() is deprecated and will be removed in version 2.0.  Use the text() construct, or the Connection.exec_driver_sql() method to invoke a driver-level SQL string. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    return connection.execute(statement, *multiparams, **params)
  /home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/engine/base.py:1639: RemovedIn20Warning: The current statement is being autocommitted using implicit autocommit.Implicit autocommit will be removed in dis_sqlalchemy 2.0.   Use the .begin() method of Engine or Connection in order to use an explicit transaction for DML and DDL statements. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    self._commit_impl(autocommit=True)
  test3.py:10: RemovedIn20Warning: The Engine.execute() function/method is considered legacy as of the 1.x series of dis_sqlalchemy and will be removed in 2.0. All statement execution in dis_sqlalchemy 2.0 is performed by the Connection.execute() method of Connection, or in the ORM by the Session.execute() method of Session. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9) (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    engine.execute("INSERT INTO foo (id) VALUES (1)")
  /home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/engine/base.py:2856: RemovedIn20Warning: Passing a string to Connection.execute() is deprecated and will be removed in version 2.0.  Use the text() construct, or the Connection.exec_driver_sql() method to invoke a driver-level SQL string. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    return connection.execute(statement, *multiparams, **params)
  /home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/engine/base.py:1639: RemovedIn20Warning: The current statement is being autocommitted using implicit autocommit.Implicit autocommit will be removed in dis_sqlalchemy 2.0.   Use the .begin() method of Engine or Connection in order to use an explicit transaction for DML and DDL statements. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    self._commit_impl(autocommit=True)
  /home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/sql/selectable.py:4271: RemovedIn20Warning: The legacy calling style of select() is deprecated and will be removed in dis_sqlalchemy 2.0.  Please use the new calling style described at select(). (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9) (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    return cls.create_legacy_select(*args, **kw)
  test3.py:14: RemovedIn20Warning: The Engine.execute() function/method is considered legacy as of the 1.x series of dis_sqlalchemy and will be removed in 2.0. All statement execution in dis_sqlalchemy 2.0 is performed by the Connection.execute() method of Connection, or in the ORM by the Session.execute() method of Session. (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9) (Background on dis_sqlalchemy 2.0 at: https://sqlalche.me/e/b8d9)
    result = engine.execute(select([foo.c.id]))
  [(1,)]
...
on is then ready to run in dis_sqlalchemy 2.0.
...
the dis_sqlalchemy project itself, the approach taken is as follows:

1. enable the ``dis_sqlalchemy_WARN_20=1`` environment variable in the test suite,
   for dis_sqlalchemy this is in the tox.ini file
...
2. Within the setup for the test suite, set up a series of warnings filters
   ...
   .. sourcecode::
        ...
        for msg in [
            ...
            r"The connection.execute\(\) method in dis_sqlalchemy 2.0 will accept "
...
Once the application passes all tests/ runs with ``dis_sqlalchemy_WARN_20=1``
...
dis_sqlalchemy 2.0 has new support for runtime interpretation of :pep:`484` typing annotations
...
dis_sqlalchemy applications that use the :ref:`Mypy plugin <mypy_toplevel>` with
...
Migration to 2.0 Step Seven - Test against a dis_sqlalchemy 2.0 Release
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

As mentioned previously, dis_sqlalchemy 2.0 has additional API and behavioral
...
of dis_sqlalchemy 2.0 to correct for any remaining issues that might be present.

The guide at :ref:`whatsnew_20_toplevel` provides an overview of
new features and behaviors for dis_sqlalchemy 2.0 which extend beyond the base
...
In dis_sqlalchemy 1.x, the following statements will automatically commit
the underlying DBAPI transaction, but in dis_sqlalchemy
...
dis_sqlalchemy's first releases were at odds with the spirit of the Python DBAPI
...
was essentially a mistake, as dis_sqlalchemy's many patterns that attempt to "hide"
...
to relational databases and ACID transactions in general.   dis_sqlalchemy 2.0 will
...
dis_sqlalchemy 2.0 will require that all database actions at every level be
...
the Python DBAPI).  "autobegin" is a new pattern in dis_sqlalchemy 1.4 that
...
context managers or decorators when dis_sqlalchemy was first created, so there were
...
implementations, and is supported by dis_sqlalchemy via the
...
* In order to handle result sets, ``Engine.execute`` returns a result object
  ...
  dis_sqlalchemy's "autoclose" behavior is well-tuned enough that users don't
  generally report any negative effects from this system, however it remains
  an overly implicit and inefficient system left over from dis_sqlalchemy's
  earliest releases.
...
relies upon the "bound metadata" pattern, which dis_sqlalchemy has tried to
de-emphasize for many years.   This was dis_sqlalchemy's very first advertised
...
Overall, the above executional patterns were introduced in dis_sqlalchemy's
...
execute method in dis_sqlalchemy 2.0 are highly simplified, removing many previously
...
on the result can be used::
    ...
    from dis_sqlalchemy.future.orm import Session
...
:func:`_sql.select` construct in dis_sqlalchemy 1.4 accepts both the legacy
...
dis_sqlalchemy has for many years developed a convention for SQL constructs
...
dis_sqlalchemy 2.0 finally resolves this by changing the :func:`_sql.select` construct
...
The ``dis_sqlalchemy.ext.declarative`` package is mostly, with some exceptions,
moved to the ``dis_sqlalchemy.orm`` package.  The :func:`_orm.declarative_base`
...
Change imports::
    ...
    from dis_sqlalchemy.ext import declarative_base, declared_attr

To::

    from dis_sqlalchemy.orm import declarative_base, declared_attr

**Discussion**

After ten years or so of popularity, the ``dis_sqlalchemy.ext.declarative``
package is now integrated into the ``dis_sqlalchemy.orm`` namespace, with the
...
The ``dis_sqlalchemy.orm.mapper()`` standalone function moves behind the scenes to
...
The biggest visible change in dis_sqlalchemy 2.0 is the use of
...
:class:`_result.Result` object, the new version of the dis_sqlalchemy
...
The fact that dis_sqlalchemy has both a :func:`_expression.select` construct
...
inconsistency in dis_sqlalchemy, one that arose as a result of small incremental
additions over time that added up to two major APIs that are divergent.

In dis_sqlalchemy's first releases, the :class:`_orm.Query` object didn't exist
...
:class:`_query.Query` came along some months / years into dis_sqlalchemy's history
...
were not present in dis_sqlalchemy previously, and the :func:`_sql.select` construct
...
In modern dis_sqlalchemy, this goal has been achieved and the two objects are now
...
dis_sqlalchemy originally included "get()" to resemble the Hibernate
...
Modern dis_sqlalchemy 1.x versions support the recommended technique which
is to use mapped attributes::

    # compatible with all modern dis_sqlalchemy versions
...
The same techniques apply to :term:`2.0-style` style use::
    ...
    # dis_sqlalchemy 1.4 / 2.0 cross compatible use
...
flag which dis_sqlalchemy developers could absolutely find no occurrence of this
...
dis_sqlalchemy 1.4 also features an improved labeling style that no longer requires
...
of view as well as how the internals of the dis_sqlalchemy ORM must handle it.
...
The situation here is a little bit unusual, in that dis_sqlalchemy is requiring
...
eager load collections are used is inconvenient, in modern dis_sqlalchemy
...
The new "write only" feature is only available in dis_sqlalchemy 2.0, and is
...
of dis_sqlalchemy.  The flag has stayed around mostly in support of allowing
...
dis_sqlalchemy 1.4 includes asyncio support for both Core and ORM.
