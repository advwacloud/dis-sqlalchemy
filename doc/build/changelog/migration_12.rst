=============================
What's New in dis_sqlalchemy 1.2?
=============================

.. admonition:: About this Document

    This document describes changes between dis_sqlalchemy version 1.1
    and dis_sqlalchemy version 1.2.


Introduction
============

This guide introduces what's new in dis_sqlalchemy version 1.2,
and also documents changes which affect users migrating
their applications from the 1.1 series of dis_sqlalchemy to 1.2.
...
dis_sqlalchemy 1.2 now moves the minimum Python version to 2.7, no longer
...
dis_sqlalchemy is currently tested on versions 3.5 and 3.6.
...
The :mod:`dis_sqlalchemy.ext.baked` extension, first introduced in the 1.0 series,
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy.orm import selectin_polymorphic
...
Both hybrid attributes (e.g. :mod:`dis_sqlalchemy.ext.hybrid`) as well as composite
...
The :class:`dis_sqlalchemy.ext.hybrid.hybrid_property` class now supports
...
if this "append" event is the second part of a bulk replace::
    ...
    from dis_sqlalchemy.orm.attributes import OP_BULK_REPLACE
...
New "modified" event handler for dis_sqlalchemy.ext.mutable
-------------------------------------------------------
...
method, which is normally called from the :mod:`dis_sqlalchemy.ext.mutable`
extension::
...
To work around this, dis_sqlalchemy and other database libraries detect this
...
dis_sqlalchemy in its early days began with this approach as well, but soon it
...
To simulate this result, dis_sqlalchemy changed from using ``1 != 1`` to
...
was added when an empty IN expression was encountered, favoring that dis_sqlalchemy
...
dis_sqlalchemy's SQL architecture is more sophisticated than it was when this
...
.. sourcecode:: pycon+sql
    ...
    >>> from dis_sqlalchemy import select, table, column, func, tuple_
...
DML statements in batch.   dis_sqlalchemy 1.2 now includes support for these
...
:class:`_expression.Insert` object, via :func:`dis_sqlalchemy.dialects.mysql.dml.insert`.
...
With the introduction of the 6.x series of the cx_Oracle DBAPI, dis_sqlalchemy's
...
* cx_Oracle's LOB datatype represents return values as a ``cx_Oracle.LOB``
  ...
  fetch". dis_sqlalchemy worked around this by both automatically calling
  ...
  can occur on dis_sqlalchemy in the case that the seldom (if ever) used
