...
=============================
What's New in dis_sqlalchemy 2.0?
=============================
...
.. admonition:: Note for Readers
    ...
    * :ref:`whatsnew_20_toplevel` - this document, new features and behaviors for dis_sqlalchemy 2.0
    ...
    :ref:`migration_20_toplevel` for a guide to ensuring dis_sqlalchemy 2.0
...
.. admonition:: About this Document
    ...
    and dis_sqlalchemy version 2.0, **independent** of the major changes between
    ...
    paradigm shift in dis_sqlalchemy 2.0 is deep integration with :pep:`484` typing
    ...
    with the composition of a dis_sqlalchemy application.
...
dis_sqlalchemy2-stubs_ package.   The new approach begins at the most fundamental
element in dis_sqlalchemy which is the :class:`_schema.Column`, or more
...
This section provides background and examples for dis_sqlalchemy's new
...
In dis_sqlalchemy2-stubs_, SQL expressions were typed as generics_ that then
...
the original Dropbox dis_sqlalchemy-stubs_ package did, where
...
of the current capabilities of :pep:`484`, dis_sqlalchemy 2.0 has realized the
original wisdom of dis_sqlalchemy-stubs_ in this area and returned to linking
...
dis_sqlalchemy construct -> Python datatype to be set up, without the need for
...
* Legacy :class:`_orm.Query` gains tuple typing as well.
  ...
  dis_sqlalchemy-stubs_ or dis_sqlalchemy2-stubs_ offered, where both scalar-object
...
A key caveat with the typing support is that **all dis_sqlalchemy stubs packages
...
However, a dis_sqlalchemy stubs package is also currently part of typeshed_, which
...
Once dis_sqlalchemy 2.0 is released in final status, typeshed will remove
dis_sqlalchemy from its own stubs source.
...
dis_sqlalchemy 1.4 introduced the first dis_sqlalchemy-native ORM typing support
using a combination of dis_sqlalchemy2-stubs_ and the :ref:`Mypy Plugin <mypy_toplevel>`.
In dis_sqlalchemy 2.0, the Mypy plugin **remains available, and has been updated
to work with dis_sqlalchemy 2.0's typing system**.  However, it should now be
...
dataclasses approach is that dis_sqlalchemy's approach is strictly **opt-in**,
...
dis_sqlalchemy's current integration approach converts the user-defined class
into a **real dataclass** to provide runtime functionality; the feature
makes use of the existing dataclass feature introduced in dis_sqlalchemy 1.4 at
...
dis_sqlalchemy historically has always needed to invoke one statement per parameter
...
historically represents dis_sqlalchemy's worst-case INSERT performance task,
...
This test can be run from any dis_sqlalchemy source tree as follows:
...
the latest 1.4 series of dis_sqlalchemy compared to 2.0, both running
the same test:
...
.. note::
   ...
   .. [#] The feature is was temporarily disabled for SQL Server in
      dis_sqlalchemy 2.0.9 due to issues with row ordering when RETURNING is used.
      In dis_sqlalchemy 2.0.10, the feature is re-enabled, with special
...
for which fast executemany was already implemented in dis_sqlalchemy 1.4,
...
dis_sqlalchemy 1.4 ported the features of the legacy :class:`_orm.Query` object to
...
with column expressions or full ORM entities::
    ...
    >>> from dis_sqlalchemy.dialects.sqlite import insert as sqlite_upsert
...
dis_sqlalchemy 1.4 also had some modest support for the RETURNING feature to be
...
The dis_sqlalchemy C extensions have been replaced with all new extensions written
...
* The Cython extensions that replace specific C extensions have all benchmarked
  ...
  virtually all the C code that dis_sqlalchemy previously
...
* Cython extensions are much easier to write, maintain and debug compared to
  ...
  code.   It is expected that many more elements of dis_sqlalchemy will be
...
* Cython is very mature and widely used, including being the basis of some
  of the prominent database drivers supported by dis_sqlalchemy including
  ``asyncpg``, ``psycopg3`` and ``asyncmy``.

Like the previous C extensions, the Cython extensions are pre-built within
dis_sqlalchemy's wheel distributions which are automatically available to ``pip``
...
versions of dis_sqlalchemy, where here we use it to illustrate differences in
...
For dis_sqlalchemy-included dialects for SQLite, PostgreSQL, MySQL/MariaDB,
...
dis_sqlalchemy's "default" driver for the ``postgresql`` dialects. ``psycopg`` is a
...
``psycopg`` is the first DBAPI supported by dis_sqlalchemy which provides
...
dis_sqlalchemy 1.4.45 where this became a :class:`.CompileError` (part of
:ticket:`8800`).
...
This section covers behavioral changes made in dis_sqlalchemy 2.0 which are
...
:ref:`session_external_transaction`, which also changed from dis_sqlalchemy 1.3
...
older dis_sqlalchemy versions as well as that it does not start a new SAVEPOINT
...
The following illustrates a case that worked in dis_sqlalchemy 1.3, stopped working
in dis_sqlalchemy 1.4, and is now restored in dis_sqlalchemy 2.0::
...
dis_sqlalchemy 1.3, the above case worked because the :class:`_orm.Session` would
...
To solve this issue, dis_sqlalchemy 2.0.4 introduces a new parameter on
...
Future dis_sqlalchemy releases may opt to provide an explicit ordering hint for the
...
Prior to dis_sqlalchemy 1.4, the :class:`.Sequence` construct would emit only
simple ``CREATE SEQUENCE`` DDL, if no additional arguments were specified:

.. sourcecode:: pycon+sql

    >>> # dis_sqlalchemy 1.3 (and 2.0)
    >>> from dis_sqlalchemy import Sequence
    >>> from dis_sqlalchemy.schema import CreateSequence
...
and pylance.  Given a program as below::
    ...
    from dis_sqlalchemy import String
    from dis_sqlalchemy.dialects.mysql import VARCHAR
...
``"mariadb"`` dialects which are considered separately as of dis_sqlalchemy 1.4.
...
"0".  For this and similar backends, dis_sqlalchemy now renders the SQL using
...
dis_sqlalchemy now renders the SQL using a form which is equivalent towards:
...
dis_sqlalchemy version, the floor function::

    expr = func.floor(literal(5, Integer) / literal(10, Integer))

The above form would be needed on any dis_sqlalchemy version prior to 2.0
...
.. sourcecode:: text
    ...
    File "/home/classic/dev/dis_sqlalchemy/test3.py", line 30, in worker
        sess.execute(select(A)).all()
    File "/home/classic/tmp/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 1691, in execute
        conn = self._connection_for_bind(bind)
    File "/home/classic/tmp/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 1532, in _connection_for_bind
        return self._transaction._connection_for_bind(
    File "/home/classic/tmp/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 754, in _connection_for_bind
...
.. sourcecode:: text
    ...
    File "/home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 1785, in close
       self._close_impl(invalidate=False)
    File "/home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 1827, in _close_impl
       transaction.close(invalidate)
    File "<string>", line 2, in close
    File "/home/classic/dev/dis_sqlalchemy/lib/dis_sqlalchemy/orm/session.py", line 506, in _go
       raise sa_exc.InvalidRequestError(
    dis_sqlalchemy.exc.InvalidRequestError: Method 'close()' can't be called here;
...
The dis_sqlalchemy :class:`_sqltypes.Float` datatype also accepts a "precision"
...
the :meth:`_types.TypeEngine.with_variant` method as follows::
    ...
    from dis_sqlalchemy.types import Float
    from dis_sqlalchemy.dialects import oracle
...
and asyncpg dialects.  The new support uses a new dis_sqlalchemy-specific
...
Existing dis_sqlalchemy projects that make use of PG-specific directives within
