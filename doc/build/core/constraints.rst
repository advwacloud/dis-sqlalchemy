...
This section will discuss SQL :term:`constraints` and indexes.  In dis_sqlalchemy
...
pairs of rows which have a relationship with each other, and dis_sqlalchemy
assigns very deep importance to this concept in virtually every area of its
operation.

In dis_sqlalchemy as well as in DDL, foreign key constraints can be defined as
...
is specified by constructing a :class:`~dis_sqlalchemy.schema.ForeignKey` object
as an argument to a :class:`~dis_sqlalchemy.schema.Column` object::
...
The argument to :class:`~dis_sqlalchemy.schema.ForeignKey` is most commonly a
...
also be an actual :class:`~dis_sqlalchemy.schema.Column` object, which as we'll
see later is accessed from an existing :class:`~dis_sqlalchemy.schema.Table`
...
:class:`~dis_sqlalchemy.schema.ForeignKeyConstraint` object. This object can
...
:class:`~dis_sqlalchemy.schema.ForeignKeyConstraint` is the only way to define a
composite foreign key. While we could also have placed individual
:class:`~dis_sqlalchemy.schema.ForeignKey` objects on both the
``invoice_item.invoice_id`` and ``invoice_item.ref_num`` columns, dis_sqlalchemy
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.CircularDependencyError: Can't sort tables for DROP;
...
.. sourcecode:: text
    ...
    dis_sqlalchemy.exc.CompileError: Can't emit DROP CONSTRAINT for constraint
...
:class:`~dis_sqlalchemy.schema.ForeignKeyConstraint` objects support the
...
``unique`` keyword on :class:`~dis_sqlalchemy.schema.Column`. Explicitly named
unique constraints and/or those with multiple columns are created via the
:class:`~dis_sqlalchemy.schema.UniqueConstraint` table-level construct.
...
Table level, using the :class:`~dis_sqlalchemy.schema.CheckConstraint` construct.
...
The :class:`_schema.Table` is the dis_sqlalchemy Core construct that allows one to define
table metadata, which among other things can be used by the dis_sqlalchemy ORM
...
by schema migration tools such as `Alembic <https://alembic.dis_sqlalchemy.org/>`_.
However, neither Alembic nor dis_sqlalchemy currently create names for constraint
...
and :paramref:`_schema.Column.index` parameters.  As of dis_sqlalchemy 0.9.2 this
...
the long-standing dis_sqlalchemy behavior of assigning a name to a :class:`.Index`
object that is created using the :paramref:`_schema.Column.index` parameter::

    >>> from dis_sqlalchemy.sql.schema import DEFAULT_NAMING_CONVENTION
...
.. seealso::
    ...
    `The Importance of Naming Constraints <https://alembic.dis_sqlalchemy.org/en/latest/naming.html>`_ - in the Alembic documentation.
...
.. autofunction:: dis_sqlalchemy.schema.conv
...
:class:`~dis_sqlalchemy.schema.Column`, which also modifies the usage of
...
more than one column, use the :class:`~dis_sqlalchemy.schema.Index` construct,
which requires a name.

Below we illustrate a :class:`~dis_sqlalchemy.schema.Table` with several
:class:`~dis_sqlalchemy.schema.Index` objects associated. The DDL for "CREATE
...
The :class:`~dis_sqlalchemy.schema.Index` object also supports its own ``create()`` method:
...
a "case insensitive" index can be created using the ``lower()`` function::
    ...
    from dis_sqlalchemy import func, Index
