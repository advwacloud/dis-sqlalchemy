...
dis_sqlalchemy includes several connection pool implementations
...
:func:`~dis_sqlalchemy.create_engine` function in most cases has a :class:`.QueuePool`
...
directly to :func:`~dis_sqlalchemy.create_engine` as keyword arguments:
...
All dis_sqlalchemy pool implementations have in common
...
imported from the ``dis_sqlalchemy.pool`` module, and handles the details
...
by any additional options::
    ...
    import dis_sqlalchemy.pool as pool
    import psycopg2
...
this state.  The two dis_sqlalchemy included dialects which are known to have
...
``logging.DEBUG``
log level along with the ``dis_sqlalchemy.pool`` logger, or by setting
...
The above pool will show verbose logging including reset on return::
    ...
    >>> c1 = engine.connect()
    DEBUG dis_sqlalchemy.pool.impl.QueuePool Created new connection <connection object ...>
    DEBUG dis_sqlalchemy.pool.impl.QueuePool Connection <connection object ...> checked out from pool
    >>> c1.close()
    DEBUG dis_sqlalchemy.pool.impl.QueuePool Connection <connection object ...> being returned to pool
    DEBUG dis_sqlalchemy.pool.impl.QueuePool Connection <connection object ...> rollback-on-return
...
behaviors are needed::
    ...
    def ping_connection(connection, branch):
        if branch:
            # this parameter is always False as of dis_sqlalchemy 2.0,
            # but is still accepted by the event hook.  In 1.x versions
            # of dis_sqlalchemy, "branched" connections should be skipped.
            return
        ...
        try:
            ...
            # catch dis_sqlalchemy's DBAPIError, which is a wrapper
...
The above recipe has the advantage that we are making use of dis_sqlalchemy's
...
to let dis_sqlalchemy handle disconnects as  they occur, at which point all
...
illustrated by the code example below::
    ...
    from dis_sqlalchemy import create_engine, exc
...
* a DBAPI exception such as :class:`.OperationalError`, raised when a
  ...
  standard system for determining the nature of an exception, all dis_sqlalchemy
...
* When a listener implementing :meth:`_events.PoolEvents.checkout` raises the
  :class:`~dis_sqlalchemy.exc.DisconnectionError` exception, indicating that the connection
...
dis_sqlalchemy dialects each include a routine called ``is_disconnect()`` that is
...
The dis_sqlalchemy :class:`_engine.Engine` object refers to a connection pool of existing
...
like in the following example::
    ...
    from dis_sqlalchemy import create_pool_from_url
    from dis_sqlalchemy import NullPool
...
.. autoclass:: dis_sqlalchemy.pool.Pool
    :members:

.. autoclass:: dis_sqlalchemy.pool.QueuePool
    :members:
