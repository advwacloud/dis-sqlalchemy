...
including :class:`~dis_sqlalchemy.schema.Table`,
:class:`~dis_sqlalchemy.schema.ForeignKeyConstraint`,
:class:`~dis_sqlalchemy.schema.CheckConstraint`, and
:class:`~dis_sqlalchemy.schema.Sequence`. Throughout, we've relied upon the
``create()`` and :func:`~dis_sqlalchemy.schema.MetaData.create_all` methods of
:class:`~dis_sqlalchemy.schema.Table` and :class:`~dis_sqlalchemy.schema.MetaData` in
...
required, dis_sqlalchemy offers two techniques which can be used to add any DDL
...
:class:`~dis_sqlalchemy.schema.DDL` construct. This construct works like all the
...
custom compilation - see :ref:`dis_sqlalchemy.ext.compiler_toplevel` for
details.
...
The ``dis_sqlalchemy.schema`` package contains SQL expression constructs that
...
Above, the :class:`~dis_sqlalchemy.schema.CreateTable` construct works like any
other expression construct (such as ``select()``, ``table.insert()``, etc.).
All of dis_sqlalchemy's DDL oriented constructs are subclasses of
...
not only in dis_sqlalchemy but in Alembic Migrations as well.
...
:ref:`dis_sqlalchemy.ext.compiler_toplevel` has several examples of this.
...
declarative mapping), such as::
    ...
    from dis_sqlalchemy import CheckConstraint, Index
    from dis_sqlalchemy import MetaData, Table, Column
    from dis_sqlalchemy import Integer, String
