...
.. module:: dis_sqlalchemy.types

dis_sqlalchemy provides abstractions for most common database data types,
...
The typical dis_sqlalchemy application will likely wish to use primarily
...
of "UPPERCASE" types in a dis_sqlalchemy application indicates that specific
...
The "UPPERCASE" datatypes that are part of ``dis_sqlalchemy.types`` are common
...
dis_sqlalchemy dialects provide **backend-specific** "UPPERCASE" datatypes, for a
...
``dis_sqlalchemy.types`` module. An example is when creating a MySQL string
...
particular type of Python data.  dis_sqlalchemy will choose the best
...
.. autoclass:: dis_sqlalchemy.types.INTEGER
