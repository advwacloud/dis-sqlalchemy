...
using the dis_sqlalchemy ORM, these objects are not generally accessed; instead,
...
.. note::
  ...
  dis_sqlalchemy ORM, the public API for transaction control is via the
...
The :class:`~dis_sqlalchemy.engine.Connection` object always emits SQL statements
...
.. topic::  the Python DBAPI is where autobegin actually happens
    ...
    that dis_sqlalchemy interacts with. In the DBAPI, the ``connection`` object does
    ...
    dis_sqlalchemy's API is basically re-stating this behavior in terms of higher
...
.. versionadded:: 2.0 "commit as you go" style is a new feature of
   dis_sqlalchemy 2.0.  It is also available in dis_sqlalchemy 1.4's "transitional"
...
.. tip::
    ...
    emitted on the :class:`_engine.Connection` until the block ends::
        ...
        ...     conn.commit()
        ...     conn.begin()
        2021-11-08 09:49:07,517 INFO dis_sqlalchemy.engine.Engine BEGIN (implicit)
        2021-11-08 09:49:07,517 INFO dis_sqlalchemy.engine.Engine COMMIT
        Traceback (most recent call last):
        ...
        dis_sqlalchemy.exc.InvalidRequestError: Can't operate on closed transaction inside
...
but it may also include other directives. dis_sqlalchemy treats the concept of
...
.. tip::
  ...
  start a transaction implicitly (which means that dis_sqlalchemy's "begin" **does
  not change autocommit mode**).

dis_sqlalchemy dialects should support these isolation levels as well as autocommit
...
.. tip::  The return value of
   ...
   object in place.  This is a new behavior as of dis_sqlalchemy 2.0.
...
DBAPI-level "autocommit" which is treated by dis_sqlalchemy as another transaction
...
**autocommitting** regardless of dis_sqlalchemy-level transaction blocks::
...
.. sourcecode:: text
    ...
    INFO dis_sqlalchemy.engine.Engine BEGIN (implicit)
    ...
    INFO dis_sqlalchemy.engine.Engine COMMIT using DBAPI connection.commit(), DBAPI should ignore due to autocommit mode

At the same time, even though we are using "DBAPI autocommit", dis_sqlalchemy's
...
transaction, and is independent of the begin/commit behavior of the dis_sqlalchemy
...
.. topic:: What we really mean is "buffered" vs. "unbuffered" results
  ...
  dis_sqlalchemy does not include any explicit support for these behaviors; within
  dis_sqlalchemy itself, the general term "server side cursors" should be considered
...
For this reason, dis_sqlalchemy's dialects will always default to the less error
...
.. versionadded:: 1.4.40 :paramref:`_engine.Connection.execution_options.yield_per` as a
   Core-only option is new as of dis_sqlalchemy 1.4.40; for prior 1.4 versions,
...
.. versionadded:: 1.4  dis_sqlalchemy now has a transparent query caching system
...
dis_sqlalchemy includes a comprehensive caching system for the SQL compiler as well
...
While dis_sqlalchemy has had a rudimentary statement cache since the early 1.x
...
an internal dis_sqlalchemy dictionary subclass that tracks the usage of particular
...
.. sourcecode:: text
  ...
  INFO dis_sqlalchemy.engine.Engine PRAGMA temp.table_info("a")
  INFO dis_sqlalchemy.engine.Engine [raw sql] ()
  INFO dis_sqlalchemy.engine.Engine PRAGMA main.table_info("b")
  INFO dis_sqlalchemy.engine.Engine [raw sql] ()
...
dis_sqlalchemy does not parse SQL strings ahead of time.

The next statements we see are the CREATE TABLE statements:

.. sourcecode:: sql

  INFO dis_sqlalchemy.engine.Engine
  CREATE TABLE a (
    id INTEGER NOT NULL,
    data VARCHAR,
    PRIMARY KEY (id)
  ...
  INFO dis_sqlalchemy.engine.Engine [no key 0.00007s] ()
  INFO dis_sqlalchemy.engine.Engine
  CREATE TABLE b (
    id INTEGER NOT NULL,
    a_id INTEGER,
  ...
  INFO dis_sqlalchemy.engine.Engine [no key 0.00006s] ()
...
.. sourcecode:: sql
  ...
  INFO dis_sqlalchemy.engine.Engine INSERT INTO a (data) VALUES (?)
  INFO dis_sqlalchemy.engine.Engine [generated in 0.00011s] (None,)
  INFO dis_sqlalchemy.engine.Engine INSERT INTO a (data) VALUES (?)
  INFO dis_sqlalchemy.engine.Engine [cached since 0.0003533s ago] (None,)
  INFO dis_sqlalchemy.engine.Engine INSERT INTO a (data) VALUES (?)
  INFO dis_sqlalchemy.engine.Engine [cached since 0.0005326s ago] (None,)
  INFO dis_sqlalchemy.engine.Engine INSERT INTO b (a_id, data) VALUES (?, ?)
  INFO dis_sqlalchemy.engine.Engine [generated in 0.00010s] (1, None)
  INFO dis_sqlalchemy.engine.Engine INSERT INTO b (a_id, data) VALUES (?, ?)
  INFO dis_sqlalchemy.engine.Engine [cached since 0.0003232s ago] (1, None)
  INFO dis_sqlalchemy.engine.Engine INSERT INTO b (a_id, data) VALUES (?, ?)
  INFO dis_sqlalchemy.engine.Engine [cached since 0.0004887s ago] (1, None)

Above, we see essentially two unique SQL strings; ``"INSERT INTO a (data) VALUES (?)"``
and ``"INSERT INTO b (a_id, data) VALUES (?, ?)"``.  Since dis_sqlalchemy uses
...
.. sourcecode:: text
  ...
  INFO dis_sqlalchemy.engine.Engine SELECT a.id AS a_id, a.data AS a_data
  FROM a
  INFO dis_sqlalchemy.engine.Engine [generated in 0.00009s] ()
  INFO dis_sqlalchemy.engine.Engine SELECT b.id AS b_id, b.a_id AS b_a_id, b.data AS b_data
  FROM b
  WHERE ? = b.a_id
  INFO dis_sqlalchemy.engine.Engine [generated in 0.00010s] (1,)
  INFO dis_sqlalchemy.engine.Engine SELECT b.id AS b_id, b.a_id AS b_a_id, b.data AS b_data
  FROM b
  WHERE ? = b.a_id
  INFO dis_sqlalchemy.engine.Engine [cached since 0.0005922s ago] (2,)
  INFO dis_sqlalchemy.engine.Engine SELECT b.id AS b_id, b.a_id AS b_a_id, b.data AS b_data
  FROM b
  WHERE ? = b.a_id
...
when using the ORM :meth:`_orm.Session.execute` method for dis_sqlalchemy-2.0
...
The dis_sqlalchemy ORM uses the above technique to hold onto per-mapper caches
...
the compiled string will not be re-usable.   dis_sqlalchemy supports rendered
...
feature, dis_sqlalchemy as of version 1.4.5 has added an attribute to dialects
...
dialect that subclasses an existing cacheable dis_sqlalchemy dialect such as
``dis_sqlalchemy.dialects.postgresql.PGDialect`` must still explicitly include this
...
dis_sqlalchemy's :ref:`post-compile <change_4808>` facility, which will render the
...
`dialect third party test suite <https://github.com/dis_sqlalchemy/dis_sqlalchemy/blob/main/README.dialects.rst>`_
...
form of a SQL expression construct as is dis_sqlalchemy's normal behavior when
...
* **Bound parameters are automatically accommodated** - in contrast to dis_sqlalchemy's
...
* **Don't use functions inside the lambda to produce bound values** - the
  ...
  attempted::
    ...
    Traceback (most recent call last):
      # ...
    dis_sqlalchemy.exc.InvalidRequestError: Can't invoke Python callable get_x()
...
* **Avoid referring to non-SQL constructs inside of lambdas as they are not
  ...
  So the following example will also raise a rather detailed error message::
    ...
    Traceback (most recent call last):
      # ...
    dis_sqlalchemy.exc.InvalidRequestError: Closure variable named 'foo' inside of
...
dis_sqlalchemy's caching system normally generates a cache key from a given
SQL expression construct by producing a structure that represents all the
state within the construct::

    >>> from dis_sqlalchemy import select, column
    >>> stmt = select(column("q"))
    >>> cache_key = stmt._generate_cache_key()
    >>> print(cache_key)  # somewhat paraphrased
    CacheKey(key=(
      '0',
      <class 'dis_sqlalchemy.sql.selectable.Select'>,
      '_raw_columns',
      (
        (
          '1',
          <class 'dis_sqlalchemy.sql.elements.ColumnClause'>,
          'name',
          'q',
          'type',
          (
            <class 'dis_sqlalchemy.sql.sqltypes.NullType'>,
          ),
        ),
      ),
...
key::
    ...
    CacheKey(key=(
      ...
      <class 'dis_sqlalchemy.sql.lambdas.StatementLambdaElement'>,
    ),)
...
represent both of these segments as well as the ``column()`` object::
    ...
    CacheKey(key=(
      ...
      <class 'dis_sqlalchemy.sql.lambdas.LinkedLambdaElement'>,
      (
        '0',
        <class 'dis_sqlalchemy.sql.elements.ColumnClause'>,
        'name',
...
As more databases have added support for INSERT..RETURNING, dis_sqlalchemy has
...
With recent support for RETURNING added to SQLite and MariaDB, dis_sqlalchemy no
...
all :ref:`dis_sqlalchemy-included <included_dialects>` backends with the exception
...
feature of the ``psycopg2`` DBAPI, which dis_sqlalchemy incrementally added more
and more support towards in recent release series.

Current Support
~~~~~~~~~~~~~~~

The feature is enabled for all backend included in dis_sqlalchemy that support
...
For dis_sqlalchemy's included dialects, support or equivalent support is currently
as follows:
...
When used by the dis_sqlalchemy ORM :term:`unit of work` process, as well as for
...
set. Unlike previous dis_sqlalchemy versions, it does so in a tight loop that
...
The "insertmanyvalues" feature integrates fully with dis_sqlalchemy's :ref:`statement
...
dis_sqlalchemy, where the production of multiple INSERT statements was hidden from
...
may be invoked.  When working with dis_sqlalchemy, textual SQL is actually more
...
There are some cases where dis_sqlalchemy does not provide a genericized way
...
While dis_sqlalchemy may in the future add built-in patterns for more DBAPI
...
dis_sqlalchemy supports calling stored procedures and user defined functions
...
normal dis_sqlalchemy connection usage. One example of this usage pattern is,
...
2. The package should have a module containing the dialect class,
   which is typically a subclass of :class:`dis_sqlalchemy.engine.default.DefaultDialect`.
...
an existing dis_sqlalchemy-supported database, the name can be given
...
dis_sqlalchemy also allows a dialect to be registered within the current process, bypassing
