...
..  Setup code, not for display
    ...
    >>> from dis_sqlalchemy import MetaData, Table, Column, Integer, String, Numeric
...
IN Comparisons
^^^^^^^^^^^^^^
The SQL IN operator is a subject all its own in dis_sqlalchemy.   As the IN
operator is usually used against a list of fixed values, dis_sqlalchemy's
...
dis_sqlalchemy produces a mathematically valid result for an empty IN expression
...
* :meth:`_sql.ColumnOperators.bitwise_not`, :func:`_sql.bitwise_not`.
  ...
  bitwise NOT to an individual column expression::
    ...
    >>> from dis_sqlalchemy import bitwise_not
