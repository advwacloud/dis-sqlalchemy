.. currentmodule:: dis_sqlalchemy.types
...
can be associated with any type::
    ...
    from dis_sqlalchemy.types import BINARY
...
:ref:`dis_sqlalchemy.ext.compiler_toplevel`, for additional examples.
...
.. note::
  ...
  type, which is customized by dis_sqlalchemy on a per-DBAPI basis to perform
  ...
  practice and dis_sqlalchemy no longer supports this as a public use case.
...
which coerces as needed::
    ...
    from dis_sqlalchemy.types import TypeDecorator, Unicode
...
many decimal places.   Here's a recipe that rounds them down::
    ...
    from dis_sqlalchemy.types import TypeDecorator, Numeric
...
binary in CHAR(16) if desired::
    ...
    from dis_sqlalchemy.types import TypeDecorator, CHAR
...
which is typically defined on the :class:`_orm.DeclarativeBase` class::
    ...
    import uuid
    from dis_sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column
...
to/from JSON.   Can be modified to use Python's builtin json encoder::
    ...
    from dis_sqlalchemy.types import TypeDecorator, VARCHAR
    import json
...
is best applied using the ``dis_sqlalchemy.ext.mutable`` extension.  For a
...
:func:`.type_coerce` before attempting to use this operator::
    ...
    from dis_sqlalchemy import type_coerce, String
...
dis_sqlalchemy allows Python functions to be invoked both when parameters are sent
...
in conjunction with :data:`~.dis_sqlalchemy.sql.expression.func`::
...
transparently::
    ...
    from dis_sqlalchemy.dialects.postgresql import BYTEA
...
dis_sqlalchemy Core defines a fixed set of expression operators available to all column expressions.
...
to integers::
    ...
    from dis_sqlalchemy import Integer, func
...
along with a :class:`.custom_op` to produce the factorial expression::
    ...
    from dis_sqlalchemy import Integer
    from dis_sqlalchemy.sql.expression import UnaryExpression
    from dis_sqlalchemy.sql import operators
...
database types not known by dis_sqlalchemy.   If only Python translation behavior
...
the introspection features described at :ref:`metadata_reflection`, dis_sqlalchemy
makes use of a fixed mapping which links the datatype information reported by a
database server to a dis_sqlalchemy datatype object.   For example, if we look
inside of a PostgreSQL schema at the definition for a particular database
column, we might receive back the string ``"VARCHAR"``.  dis_sqlalchemy's
PostgreSQL dialect has a hardcoded mapping which links the string name
``"VARCHAR"`` to the dis_sqlalchemy :class:`.VARCHAR` class, and that's how when we
...
.. sourcecode:: pycon+sql
    ...
    {execsql}INFO dis_sqlalchemy.engine.base.Engine
...
.. sourcecode:: pycon+sql
    ...
    {execsql}INFO dis_sqlalchemy.engine.base.Engine PRAGMA main.table_info("my_table")
    INFO dis_sqlalchemy.engine.base.Engine ()
    DEBUG dis_sqlalchemy.engine.base.Engine Col ('cid', 'name', 'type', 'notnull', 'dflt_value', 'pk')
    DEBUG dis_sqlalchemy.engine.base.Engine Row (0, 'id', 'INTEGER', 0, None, 0)
    DEBUG dis_sqlalchemy.engine.base.Engine Row (1, 'data', 'BLOB', 0, None, 0)
