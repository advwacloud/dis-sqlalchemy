...
.. seealso::
    ...
    dis_sqlalchemy's database metadata concept in the :ref:`unified_tutorial`

A collection of metadata entities is stored in an object aptly named
:class:`~dis_sqlalchemy.schema.MetaData`::

    from dis_sqlalchemy import MetaData

    metadata_obj = MetaData()

:class:`~dis_sqlalchemy.schema.MetaData` is a container object that keeps together
many different features of a database (or multiple databases) being described.

To represent a table, use the :class:`~dis_sqlalchemy.schema.Table` class. Its two
primary arguments are the table name, then the
:class:`~dis_sqlalchemy.schema.MetaData` object which it will be associated with.
The remaining positional arguments are mostly
:class:`~dis_sqlalchemy.schema.Column` objects describing each column::

    from dis_sqlalchemy import Table, Column, Integer, String
...
to genericized types, such as :class:`~dis_sqlalchemy.types.Integer` and
:class:`~dis_sqlalchemy.types.String`. dis_sqlalchemy features dozens of types of
...
The :class:`~dis_sqlalchemy.schema.MetaData` object contains all of the schema
...
list of each :class:`~dis_sqlalchemy.schema.Table` object in order of foreign key
...
In most cases, individual :class:`~dis_sqlalchemy.schema.Table` objects have been
explicitly declared, and these objects are typically accessed directly as
module-level variables in an application. Once a
:class:`~dis_sqlalchemy.schema.Table` has been defined, it has a full set of
accessors which allow inspection of its properties. Given the following
:class:`~dis_sqlalchemy.schema.Table` definition::
...
Note the :class:`~dis_sqlalchemy.schema.ForeignKey` object used in this table -
...
Once you've defined some :class:`~dis_sqlalchemy.schema.Table` objects, assuming
...
skip this section - dis_sqlalchemy has no requirement that it be used to create
your tables).

The usual way to issue CREATE is to use
:func:`~dis_sqlalchemy.schema.MetaData.create_all` on the
:class:`~dis_sqlalchemy.schema.MetaData` object. This method will issue queries
...
:func:`~dis_sqlalchemy.schema.MetaData.create_all` creates foreign key constraints
...
:func:`~dis_sqlalchemy.schema.MetaData.drop_all` method. This method does the
exact opposite of :func:`~dis_sqlalchemy.schema.MetaData.create_all` - the
...
``drop()`` methods of :class:`~dis_sqlalchemy.schema.Table`. These methods by
...
While dis_sqlalchemy directly supports emitting CREATE and DROP statements for
...
scope of dis_sqlalchemy itself.  While it's easy enough to emit ALTER statements
...
The dis_sqlalchemy project offers the  `Alembic <https://alembic.dis_sqlalchemy.org>`_
...
Alembic supersedes the `dis_sqlalchemy-Migrate
<https://github.com/openstack/dis_sqlalchemy-migrate>`_   project, which is the
original migration tool for dis_sqlalchemy and is now  considered legacy.
...
of referring to this alternate set of tables using a string name.  dis_sqlalchemy
refers to this name as the **schema name**.  Within dis_sqlalchemy, this is nothing
...
specify their own name.  Finally, dis_sqlalchemy also supports a "dynamic" schema name
...
.. topic::  What's "schema" ?
    ...
    dis_sqlalchemy's support for database "schema" was designed with first party
    ...
    in use, dis_sqlalchemy uses the phrase "schema" to refer to the qualifying
...
The schema feature of dis_sqlalchemy interacts with the table reflection
...
:class:`~dis_sqlalchemy.schema.Table` supports database-specific options. For
example, MySQL has different table backend types, including "MyISAM" and
"InnoDB". This can be expressed with :class:`~dis_sqlalchemy.schema.Table` using
``mysql_engine``::
...
.. attribute:: dis_sqlalchemy.schema.BLANK_SCHEMA
    :noindex:

    Refers to :attr:`.SchemaConst.BLANK_SCHEMA`.

.. attribute:: dis_sqlalchemy.schema.RETAIN_SCHEMA
    :noindex:
