...
emitted by dis_sqlalchemy; they may also be client-side Python functions or
constant values which are invoked by dis_sqlalchemy before data is passed to the
database.
...
.. note::
    ...
    by the application before being sent to the database.  dis_sqlalchemy provides
...
dis_sqlalchemy provides an array of features regarding default generation
...
key columns, dis_sqlalchemy will in most cases use these capabilities
automatically. See the API documentation for
:class:`~dis_sqlalchemy.schema.Column` including the :paramref:`_schema.Column.autoincrement` flag, as
well as the section on :class:`~dis_sqlalchemy.schema.Sequence` later in this
...
following) - dis_sqlalchemy will execute the function at the time the statement
executes.
...
internal dis_sqlalchemy object which contains all information about the statement
...
:paramref:`_schema.Column.onupdate` are invoked explicitly by dis_sqlalchemy when an
...
primary key columns, there are some cases where dis_sqlalchemy must "pre-execute"
...
:class:`~dis_sqlalchemy.engine.CursorResult` will contain a collection accessible
via :meth:`_engine.CursorResult.postfetch_cols` which contains a list of all
:class:`~dis_sqlalchemy.schema.Column` objects which had an inline-executed
...
:class:`~dis_sqlalchemy.engine.CursorResult`. The
...
raw value, as dis_sqlalchemy does not typically perform any quoting or escaping on
these values.
...
may be called out using :class:`.FetchedValue` as a marker::
    ...
    from dis_sqlalchemy.schema import FetchedValue
...
dis_sqlalchemy ORM then make use of this marker in order to know how to get at the
...
dis_sqlalchemy represents database sequences using the
:class:`~dis_sqlalchemy.schema.Sequence` object, which is considered to be a
special case of "column default". It only has an effect on databases which have
explicit support for sequences, which among dis_sqlalchemy's included dialects
includes PostgreSQL, Oracle, MS SQL Server, and MariaDB.  The
:class:`~dis_sqlalchemy.schema.Sequence` object is otherwise ignored.
...
The :class:`~dis_sqlalchemy.schema.Sequence` may be placed on any column as a
...
When the :class:`~dis_sqlalchemy.schema.Sequence` is associated with a
...
RETURNING for INSERT statements, which is used automatically by dis_sqlalchemy
when emitting this statement.  However if RETURNING is not used for a particular
insert, then dis_sqlalchemy would prefer to "pre-execute" the sequence outside
...
Example::
    ...
    from dis_sqlalchemy import Table, Column, MetaData, Integer, Computed
...
.. note:: A :class:`_schema.Column` that is defined with the :class:`.Computed`
   ...
   to it;  dis_sqlalchemy's behavior when a value is passed for such a column
...
Example::
    ...
    from dis_sqlalchemy import Table, Column, MetaData, Integer, Identity, String
...
and the default dis_sqlalchemy logic for autoincrementing columns is used.
