...
The :class:`_engine.Engine` is the starting point for any dis_sqlalchemy application. It's
"home base" for the actual database and its :term:`DBAPI`, delivered to the dis_sqlalchemy
...
dis_sqlalchemy includes many :class:`.Dialect` implementations for various
backends.   Dialects for the most common databases are included with dis_sqlalchemy; a handful
...
Dialect names include the identifying name of the dis_sqlalchemy dialect,
...
such as those within passwords may be passed without any modification::
    ...
    from dis_sqlalchemy import URL
...
.. autofunction:: dis_sqlalchemy.create_engine

.. autofunction:: dis_sqlalchemy.engine_from_config

.. autofunction:: dis_sqlalchemy.create_mock_engine

.. autofunction:: dis_sqlalchemy.engine.make_url

.. autofunction:: dis_sqlalchemy.create_pool_from_url

.. autoclass:: dis_sqlalchemy.engine.URL
    :members:
...
by dis_sqlalchemy dialect. Some dialects pass all arguments through as strings,
...
the ``True`` symbol and the dis_sqlalchemy dialect is not aware of this keyword
...
For a DBAPI connection that dis_sqlalchemy creates without issue, but where we
...
implement informational and debug log output with dis_sqlalchemy. This allows
dis_sqlalchemy's logging to integrate in a standard way with other applications
...
logging performed by dis_sqlalchemy exists underneath the ``dis_sqlalchemy``
namespace, as used by ``logging.getLogger('dis_sqlalchemy')``. When logging has
...
* ``dis_sqlalchemy.engine`` - controls SQL echoing.  Set to ``logging.INFO`` for
...
* ``dis_sqlalchemy.pool`` - controls connection pool logging.  Set to
...
* ``dis_sqlalchemy.dialects`` - controls custom logging for SQL dialects, to the
  extend that logging is used within specific dialects, which is generally
  minimal.

* ``dis_sqlalchemy.orm`` - controls logging of various ORM functions to the extent
...
``echo=True`` flag::
    ...
    logging.basicConfig()
    logging.getLogger("dis_sqlalchemy.engine").setLevel(logging.INFO)

By default, the log level is set to ``logging.WARN`` within the entire
``dis_sqlalchemy`` namespace so that no log operations occur, even within an
application that has logging enabled otherwise.

.. note::

   The dis_sqlalchemy :class:`_engine.Engine` conserves Python function call
...
parameters are a shortcut to immediate logging to ``sys.stdout``::
    ...
    >>> from dis_sqlalchemy import create_engine, text
    >>> e = create_engine("sqlite://", echo=True, echo_pool="debug")
    >>> with e.connect() as conn:
    ...     print(conn.scalar(text("select 'hi'")))
    2020-10-24 12:54:57,701 DEBUG dis_sqlalchemy.pool.impl.SingletonThreadPool Created new connection <sqlite3.Connection object at 0x7f287819ac60>
    2020-10-24 12:54:57,701 DEBUG dis_sqlalchemy.pool.impl.SingletonThreadPool Connection <sqlite3.Connection object at 0x7f287819ac60> checked out from pool
    2020-10-24 12:54:57,702 INFO dis_sqlalchemy.engine.Engine select 'hi'
    2020-10-24 12:54:57,702 INFO dis_sqlalchemy.engine.Engine ()
    hi
    2020-10-24 12:54:57,703 DEBUG dis_sqlalchemy.pool.impl.SingletonThreadPool Connection <sqlite3.Connection object at 0x7f287819ac60> being returned to pool
    2020-10-24 12:54:57,704 DEBUG dis_sqlalchemy.pool.impl.SingletonThreadPool Connection <sqlite3.Connection object at 0x7f287819ac60> rollback-on-return
...
Use of these flags is roughly equivalent to::
    ...
    logging.basicConfig()
    logging.getLogger("dis_sqlalchemy.engine").setLevel(logging.INFO)
    logging.getLogger("dis_sqlalchemy.pool").setLevel(logging.DEBUG)
...
The logger name of instance such as an :class:`~dis_sqlalchemy.engine.Engine` or
:class:`~dis_sqlalchemy.pool.Pool` defaults to using a truncated hex identifier
...
:func:`dis_sqlalchemy.create_engine`::
    ...
    2020-10-24 12:47:04,291 INFO dis_sqlalchemy.engine.Engine.myengine select 'hi'
    2020-10-24 12:47:04,292 INFO dis_sqlalchemy.engine.Engine.myengine ()
...
tokens::
    ...
    2021-02-03 11:48:45,754 INFO dis_sqlalchemy.engine.Engine [track1] select 1
    2021-02-03 11:48:45,754 INFO dis_sqlalchemy.engine.Engine [track1] [raw sql] ()
    2021-02-03 11:48:45,754 DEBUG dis_sqlalchemy.engine.Engine [track1] Col ('1',)
    2021-02-03 11:48:45,755 DEBUG dis_sqlalchemy.engine.Engine [track1] Row (1,)
...
of an application without creating new engines::
    ...
    ...     conn.execute("select 1").all()
    2021-02-03 11:51:08,960 INFO dis_sqlalchemy.engine.Engine [track1] select 1
    2021-02-03 11:51:08,960 INFO dis_sqlalchemy.engine.Engine [track1] [raw sql] ()
    2021-02-03 11:51:08,960 DEBUG dis_sqlalchemy.engine.Engine [track1] Col ('1',)
    2021-02-03 11:51:08,961 DEBUG dis_sqlalchemy.engine.Engine [track1] Row (1,)

    >>> with e2.connect() as conn:
    ...     conn.execute("select 2").all()
    2021-02-03 11:52:05,518 INFO dis_sqlalchemy.engine.Engine [track2] Select 1
    2021-02-03 11:52:05,519 INFO dis_sqlalchemy.engine.Engine [track2] [raw sql] ()
    2021-02-03 11:52:05,520 DEBUG dis_sqlalchemy.engine.Engine [track2] Col ('1',)
    2021-02-03 11:52:05,520 DEBUG dis_sqlalchemy.engine.Engine [track2] Row (1,)
...
:paramref:`_sa.create_engine.hide_parameters` flag::
    ...
    2020-10-24 12:48:32,808 INFO dis_sqlalchemy.engine.Engine select ?
    2020-10-24 12:48:32,808 INFO dis_sqlalchemy.engine.Engine [SQL parameters hidden due to hide_parameters=True]
