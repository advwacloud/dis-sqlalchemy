...
DISABLE_EXTENSION = bool(os.environ.get("DISABLE_dis_sqlalchemy_CEXT"))
REQUIRE_EXTENSION = bool(os.environ.get("REQUIRE_dis_sqlalchemy_CEXT"))

if DISABLE_EXTENSION and REQUIRE_EXTENSION:
    raise RuntimeError(
        "Cannot set both 'DISABLE_dis_sqlalchemy_CEXT' and "
        "'REQUIRE_dis_sqlalchemy_CEXT' environment variables"
    )
...
if HAS_CYTHON and IS_CPYTHON and not DISABLE_EXTENSION:
    ...
    module_prefix = "dis_sqlalchemy.cyextension."
    source_prefix = "lib/dis_sqlalchemy/cyextension/"
    ...
    raise RuntimeError(
        "Cython extension build is required because REQUIRE_dis_sqlalchemy_CEXT "
